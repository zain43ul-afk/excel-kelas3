/* ============================================================
   ...
   Optimized bundle.
   Stability fix 2026-09-21:
   - heartbeat siswa tidak lagi memicu full re-render dashboard Algoritma
   - start/durasi memakai pembaruan DOM ringan
   - jumlah tim 3/4/5 dirender native sejak awal
   - dashboard race hanya memperbarui jam tiap detik
   - observer modul guru dipersempit ke teacher/student view

   Bagian 1: app.js
   Bagian 2: algoritm.js V41 + embedded algorithm-team-game.js
...
(() => {
  ...
  const EXERCISES = [
    ...
    ["Menulis Rumus, Kata & Link","40 soal: rumus, kata, tautan, dan paragraf dengan penulisan yang harus tepat."],
  ...
  const L3_ITEMS = [
    ...
    "https://www.bing.com",
    "Microsoft Excel digunakan untuk mengolah data dalam bentuk tabel. Rumus membantu pengguna menghitung data secara cepat dan tepat.",
    "Setiap sel pada lembar kerja memiliki alamat yang unik. Alamat sel tersusun dari huruf kolom dan nomor baris.",
    "Fungsi SUM digunakan untuk menjumlahkan beberapa nilai. Fungsi AVERAGE digunakan untuk mencari nilai rata-rata.",
    "Data yang rapi lebih mudah dibaca dan dianalisis. Gunakan judul kolom yang jelas agar informasi tidak membingungkan.",
    "Internet membantu siswa mencari informasi untuk belajar. Gunakan sumber yang tepercaya dan periksa kembali informasi yang ditemukan.",
    "Kata sandi yang kuat sebaiknya sulit ditebak oleh orang lain. Jangan membagikan kata sandi kepada teman atau orang yang tidak berkepentingan.",
    "Algoritma adalah urutan langkah untuk menyelesaikan suatu masalah. Langkah-langkahnya harus disusun secara logis agar menghasilkan keluaran yang benar.",
    "File tugas perlu disimpan dengan nama yang mudah dikenali. Pilih folder yang tepat agar file dapat ditemukan kembali dengan cepat.",
    "Kerja kelompok membutuhkan komunikasi yang jelas dan saling menghargai. Setiap anggota perlu menyampaikan ide serta mendengarkan pendapat teman.",
    "Sebelum mengirim tugas, periksa kembali isi dan format dokumen. Pastikan tidak ada kesalahan penulisan atau bagian penting yang terlewat."
  ];
  ...
  function initFirebase(){
    ...
    try{
      ...
      db.ref("kelasExcel/progress").on("value", snap=>{
        ...
        // Satu kelas dapat mengirim banyak perubahan hampir bersamaan.
        // Menulis seluruh progress ke localStorage pada setiap event membuat
        // thread UI tersendat, jadi gabungkan burst update menjadi satu write.
  ...
  function showView(id){
    ...
    document.body.classList.toggle("home-view-active",id==="#homeView");
    document.body.classList.toggle("student-menu-active",id==="#studentView");
    document.body.classList.toggle("teacher-dashboard-active",id==="#teacherView");
  }
  ...
  function updateLiveScoreDisplays(){
    ...
    $$("[data-live-active-score]").forEach(el=>{
      if(currentExercise!==6) el.textContent=String(scoreForExercise(currentExercise));
    });
    ...
    if(exTimer && currentExercise!==6) exTimer.textContent=formatTime(getExerciseRemaining(currentExercise));
  }
  ...
  function renderStudent(){
    ...
    const desc=[
      ...
      "Salin rumus, kata, tautan, dan paragraf dengan tepat.",
    ...
    $("#studentExerciseNav").innerHTML=EXERCISES.map((e,i)=>{
      ...
      return `<button type="button" data-ex="${i}" class="student-window-card theme-${themes[i]} ${open?"open":"locked"} ${done?"done":""}">
        ...
        <small>${desc[i]}</small>
        <span class="student-window-decor" aria-hidden="true">${decorIcons[i]}</span>
  ...
  function exerciseHeader(i,extra=""){
    ...
    const scoreCard=i===6
      ? `<div class="exercise-score-card alg-algorithm-time-card"><small>WAKTU</small><strong id="algHeaderTime">--:--</strong><span>Menunggu durasi guru</span></div>`
    ...
    const progressTime=i===6?"--:--":formatTime(getExerciseRemaining(i));
    return `<div class="exercise-window-header">
      ...
      <div class="exercise-window-progress">
        ...
        <small>Sisa waktu: <b id="exerciseWindowTimer">${progressTime}</b></small>
      </div>
    </div>
  ...
  function styledWindowHeader(index,title,themeClass=""){
    ...
    const progressTime=index===6?"--:--":formatTime(getExerciseRemaining(index));
    return `<div class="l2-window-header lx-window-header ${themeClass}">
      ...
      ${scoreCard}
      <div class="l2-window-progress"><div><span>Progres keseluruhan</span><b>${percent}%</b></div><span><i style="width:${percent}%"></i></span><small>Sisa waktu: <b id="exerciseWindowTimer">${progressTime}</b></small></div>
    </div>`;
  }
  ...
  function renderL3(panel){
    ...
    st.values ||= {};
    st.guideSlide=Number.isInteger(st.guideSlide)?st.guideSlide:0;
    const totalItems=L3_ITEMS.length;
    ...
    const sections=[
      ...
      {title:"D. Menulis Paragraf",desc:"Ketik ulang 10 paragraf sama persis. Setiap paragraf terdiri dari 2 kalimat; perhatikan kapitalisasi, spasi, titik, tanda hubung, dan seluruh tanda baca.",start:30,end:40,multiline:true}
    ];
    ...
    const guides=[
      ...
      {title:"5. Tulis paragraf dua kalimat",text:"Pada bagian paragraf, salin kedua kalimat dalam satu paragraf yang sama. Huruf kapital, satu spasi setelah titik, tanda baca, dan ejaan harus sama persis dengan target.",kind:"paragraph"},
      {title:"6. Pastikan semua indikator benar",text:`Setelah ${totalItems} jawaban sama persis, tombol Selesai akan aktif. Klik Selesai untuk mengirim hasil ke dashboard guru.`,kind:"finish"}
    ];
    ...
    const guideVisual={
      ...
      paragraph:`<div class="lx-mini-paragraph"><small>Contoh paragraf 2 kalimat</small><p>Data yang rapi mudah dibaca. Informasi yang jelas membantu proses belajar.</p></div>`,
      finish:`<div class="lx-mini-success">✓ ${totalItems} / ${totalItems} jawaban benar</div>`
    }[g.kind];
    ...
    const sectionHtml=sections.map(sec=>{
      ...
      return `<section class="l2-section-card">
        <div class="l2-section-head"><div><h3>${sec.title}</h3><p>${sec.desc}</p></div><span data-l3-section-count="${sec.start}">${sectionCorrect}/${indices.length} benar</span></div>
        <div class="lx-typing-grid">${indices.map(i=>{
          const target=L3_ITEMS[i],v=st.values[i]??"",ok=v===target;
          const paragraph=!!sec.multiline;
          const field=paragraph
            ? `<textarea data-type="${i}" rows="4" autocomplete="off" spellcheck="false" placeholder="Ketik paragraf 2 kalimat persis di sini">${escapeHtml(v)}</textarea>`
            : `<input data-type="${i}" value="${escapeHtml(v)}" autocomplete="off" spellcheck="false" placeholder="Ketik persis di sini">`;
          return `<div class="lx-typing-card ${paragraph?'lx-paragraph-card ':''}${v?(ok?'correct':'wrong'):''}" data-l3-card="${i}">
            <div class="lx-card-head"><span>${i+1}</span><strong class="l3-protected-target" data-l3-protected-target="1" oncopy="return false" oncut="return false" ondragstart="return false" oncontextmenu="return false">${escapeHtml(target)}</strong></div>
            ${field}
    ...
    panel.innerHTML=`
      ...
      <div class="l2-layout">
        <main class="l2-main">
          <section class="l2-learning-row">
            <div class="l2-material-card"><h4>📖 &nbsp; Materi singkat</h4><p>Jawaban harus sama persis dengan target. Perhatikan <b>kapitalisasi</b>, <b>spasi</b>, <b>simbol</b>, dan <b>tanda baca</b>.</p><div><code>Caps Lock → kapital berulang</code><code>Shift → kapital/simbol sementara</code><code>https:// → awal tautan</code><code>Paragraf → 2 kalimat persis</code></div></div>
          ...
          <section class="l2-instruction-band">Ketik seluruh <b>${totalItems} soal</b> sama persis. Bagian paragraf berisi <b>2 kalimat per soal</b>. Indikator hijau muncul jika penulisan sudah benar.</section>
          ${sectionHtml}
          <button id="l3FinishBtn" class="l2-check-all ${correctCount===totalItems?'ready':''}" ${correctCount===totalItems?'':'disabled'}>${done?'✓ Latihan selesai':correctCount===totalItems?'✓ Selesai':`Benar ${correctCount} / ${totalItems}`}</button>
        </main>
        <aside class="l2-sidebar">
          ${styledParticipant(2)}
          ...
          <section class="l2-quick-tip"><h4>Petunjuk cepat</h4><p>Periksa kapitalisasi, spasi, dan tanda baca sebelum lanjut. Untuk paragraf, kedua kalimat harus berada dalam satu jawaban dan sama persis.</p><div>SUM ≠ sum &nbsp; • &nbsp; https:// lengkap &nbsp; • &nbsp; titik + satu spasi</div></section>
        </aside>
      </div>`;
    ...
    const refreshCounts=()=>{
      ...
      sections.forEach(sec=>{
        ...
        if(el){
          ...
          const c=Array.from({length:len},(_,j)=>sec.start+j).filter(i=>st.values[i]===L3_ITEMS[i]).length;
          el.textContent=`${c}/${len} benar`;
        }
      });
      const btn=panel.querySelector('#l3FinishBtn');
      if(btn){
        btn.disabled=cnt!==totalItems;
        btn.classList.toggle('ready',cnt===totalItems);
        btn.textContent=done?'✓ Latihan selesai':cnt===totalItems?'✓ Selesai':`Benar ${cnt} / ${totalItems}`;
      }
    };
  ...
  function teacherLogin(){
    ...
    // Saat Firebase aktif, dashboard sudah mendapat update real-time.
    // Full render tiap 8 detik hanya menambah DOM churn dan membuat kontrol
    // Algoritma terasa sulit diklik ketika banyak siswa aktif.
    if(!realtime){
  ...
  function renderTeacher(mode="full"){
  ...
  function scheduleTeacherRender(){
    ...
    // Batasi full render dashboard guru maksimal sekitar 1x per 700 ms.
    // Event progress dari puluhan siswa tetap terkumpul, tetapi UI utama
    // tidak dibangun ulang puluhan kali dalam satu detik.
    const elapsed=Date.now()-teacherLastRenderAt;
    const wait=Math.max(80,700-elapsed);

    teacherRenderTimer=setTimeout(()=>{
      teacherRenderTimer=null;
      if(!$("#teacherView").classList.contains("active"))return;
      teacherLastRenderAt=Date.now();
      renderTeacher("progress");
    },wait);
  }

  function renderReactive(source="all"){
    if($("#teacherView").classList.contains("active")){
      if(source==="progress") scheduleTeacherRender();
      else{
...
/* ==========================================================
   V42 — tinggi aman topbar untuk jendela latihan
   ========================================================== */
(()=>{
  const CSS_VAR="--app-topbar-height";
  let resizeObserver=null;
  let frameId=0;

  function syncExerciseTopbarHeight(){
    const topbar=document.querySelector(".topbar");
    const root=document.documentElement;
    if(!topbar||!root?.style)return;

    if(frameId)cancelAnimationFrame(frameId);
    frameId=requestAnimationFrame(()=>{
      frameId=0;
      const height=Math.max(0,Math.ceil(topbar.getBoundingClientRect().height));
      if(height)root.style.setProperty(CSS_VAR,`${height}px`);
    });
  }

  function initExerciseTopbarOffset(){
    const topbar=document.querySelector(".topbar");
    if(!topbar)return;

    syncExerciseTopbarHeight();
    window.addEventListener("resize",syncExerciseTopbarHeight,{passive:true});
    document.addEventListener("fullscreenchange",syncExerciseTopbarHeight);

    if("ResizeObserver" in window){
      resizeObserver=new ResizeObserver(syncExerciseTopbarHeight);
      resizeObserver.observe(topbar);
    }

    window.ExerciseHeaderAccessV42={
      version:"exercise-header-access-v42",
      sync:syncExerciseTopbarHeight,
      get offset(){
  ...
  if(document.readyState==="loading"){
    document.addEventListener("DOMContentLoaded",initExerciseTopbarOffset,{once:true});
  }else{
    initExerciseTopbarOffset();
  }
})();

;/* ================= V41 LATE JOIN MANAGER ================= */
(() => {
  "use strict";

  const VERSION="algorithm-late-join-v41";
  const HEARTBEAT_MAX_AGE=70000;
  const TEAM_CAPACITY=6;
  const STATE={
    cls:"",manager:{},lobby:{},groups:{},managerRef:null,lobbyRef:null,
    groupRefs:[],busy:false,timer:null,activeSignature:""
  };
  ...
  function sessionId(){
    try{return String(JSON.parse(localStorage.getItem("kelasExcelSettings")||"{}").sessionId||"default-session")}
    catch(_){return "default-session"}
  }

  function selectedClass(){
    return document.querySelector("[data-teacher-class].active")?.dataset.teacherClass
      ||document.querySelector("[data-teacher-class]")?.dataset.teacherClass||"3P";
  }
  ...
  function teams(){
    ...
    return Array.from({length:teamCount()},(_,i)=>{
      const value=raw[i]??raw[String(i)]??raw[i+1]??raw[String(i+1)];
  ...
  function activeStudents(){
    ...
    return [...new Set(Object.values(STATE.lobby||{})
      .filter(item=>item&&item.name&&current-Number(item.lastSeen||0)<=HEARTBEAT_MAX_AGE)
      .map(item=>item.name))];
  }

  function allLateRecords(){
    return {...(STATE.manager?.lateQueue||{}),...(STATE.manager?.lateMembers||{})};
  }

  function isRaceOpen(){
    if(!STATE.manager?.started||STATE.manager?.stopped||STATE.manager?.scoreDisabled)return false;
    const start=Number(STATE.manager.startedAt||0);
    const duration=Math.max(1,Number(STATE.manager.duration||60))*60000;
    return !start||Date.now()<start+duration;
  }

  function currentRoundIndex(teamNo){
    return Math.max(0,Number(STATE.groups?.[teamNo]?.race?.roundIndex||0));
  }

  function chooseTeam(excludeKey=""){
    const currentTeams=teams();
    const queue=Object.entries(STATE.manager?.lateQueue||{})
      .filter(([key,item])=>key!==excludeKey&&item&&item.status==="waiting_next_round");
    const loads=currentTeams.map((members,i)=>members.length+queue.filter(([,item])=>Number(item.assignedTeam)===i+1).length);
    let index=-1;
    loads.forEach((load,i)=>{
      if(load>=TEAM_CAPACITY)return;
      if(index<0||load<loads[index])index=i;
    });
    return index+1;
  }

  async function registerLateStudent(name){
    if(!name||!STATE.managerRef||!isRaceOpen())return;
    const key=safeId(name);
    if(allLateRecords()[key])return;

    await STATE.managerRef.transaction(current=>{
      const data=current||{};
      if(!data.started||data.stopped||data.scoreDisabled)return;
      const count=[3,4,5].includes(Number(data.teamCount))?Number(data.teamCount):5;
      const raw=data.teams||{};
      const currentTeams=Array.from({length:count},(_,i)=>{
        const value=raw[i]??raw[String(i)]??raw[i+1]??raw[String(i+1)];
        if(Array.isArray(value))return value.filter(Boolean);
        if(value&&typeof value==="object")return Object.values(value).filter(Boolean);
        return [];
      });
      if(currentTeams.some(team=>team.includes(name)))return data;
      data.lateQueue=data.lateQueue||{};
      data.lateMembers=data.lateMembers||{};
      if(data.lateQueue[key]||data.lateMembers[key])return data;

      const waiting=Object.values(data.lateQueue).filter(item=>item&&item.status==="waiting_next_round");
      const loads=currentTeams.map((members,i)=>members.length+waiting.filter(item=>Number(item.assignedTeam)===i+1).length);
      let teamIndex=-1;
      loads.forEach((load,i)=>{
        if(load>=TEAM_CAPACITY)return;
        if(teamIndex<0||load<loads[teamIndex])teamIndex=i;
      });
      data.lateQueue[key]={
        name,className:STATE.cls,joinedAt:Date.now(),status:"waiting_next_round",
        assignedTeam:teamIndex+1,
        effectiveRoundIndex:teamIndex>=0?currentRoundIndex(teamIndex+1)+1:null,
        penalty:10,penaltyWaived:false,reason:"late_after_start"
      };
      return data;
    });
  }

  async function reconcileLateStudents(){
    if(STATE.busy||!STATE.managerRef||!isRaceOpen())return;
    STATE.busy=true;
    try{
      const assigned=new Set(teams().flat());
      const known=allLateRecords();
      const newcomers=activeStudents().filter(name=>!assigned.has(name)&&!known[safeId(name)]);
      for(const name of newcomers)await registerLateStudent(name);
    }catch(err){
      console.warn("[V41] Sinkronisasi siswa terlambat gagal:",err);
    }finally{
      STATE.busy=false;
    }
  }
  ...
  function ensureCss(){
    if(document.getElementById("algLateJoinV41Css"))return;
    const style=document.createElement("style");
    style.id="algLateJoinV41Css";
    style.textContent=`
      #algorithmTeacherMonitor .alg-late-v41{
        margin:16px 0;padding:17px;border:1px solid #ead8a7;border-radius:18px;
        background:linear-gradient(135deg,#fffdf6,#fff8e7);box-shadow:0 8px 20px rgba(112,79,14,.06)
      }
      #algorithmTeacherMonitor .alg-late-v41-head{display:flex;justify-content:space-between;gap:14px;align-items:flex-start}
      #algorithmTeacherMonitor .alg-late-v41-title{display:flex;gap:11px;align-items:center}
      #algorithmTeacherMonitor .alg-late-v41-icon{width:42px;height:42px;display:grid;place-items:center;border-radius:13px;background:#ffe9ab;color:#956500;font-size:18px}
      #algorithmTeacherMonitor .alg-late-v41 h4{margin:0;color:#4b3915;font-size:14px;font-weight:950}
      #algorithmTeacherMonitor .alg-late-v41-head p{margin:4px 0 0;color:#866f3e;font-size:9px;line-height:1.5}
      #algorithmTeacherMonitor .alg-late-v41-count{padding:6px 10px;border-radius:999px;background:#fff0c6;color:#936300;font-size:9px;font-weight:950;white-space:nowrap}
      #algorithmTeacherMonitor .alg-late-v41-empty{margin-top:12px;padding:11px;border:1px dashed #e8d8ae;border-radius:12px;color:#8c7950;background:rgba(255,255,255,.6);font-size:9px;text-align:center}
      #algorithmTeacherMonitor .alg-late-v41-list{display:grid;gap:8px;margin-top:13px}
      #algorithmTeacherMonitor .alg-late-v41-row{display:grid;grid-template-columns:minmax(150px,1.2fr) minmax(145px,.9fr) minmax(125px,.8fr) auto;gap:9px;align-items:center;padding:10px;border:1px solid #eadfbe;border-radius:13px;background:#fff}
      #algorithmTeacherMonitor .alg-late-v41-student b{display:block;color:#493c23;font-size:10px}
      #algorithmTeacherMonitor .alg-late-v41-student small{display:block;margin-top:3px;color:#9a865e;font-size:8px}
      #algorithmTeacherMonitor .alg-late-v41-status{display:inline-flex;align-items:center;gap:5px;color:#886517;font-size:8px;font-weight:900}
      #algorithmTeacherMonitor .alg-late-v41-status.active{color:#167452}
      #algorithmTeacherMonitor .alg-late-v41 select{width:100%;min-height:34px;border:1px solid #dfd4b5;border-radius:9px;background:#fff;color:#59492d;padding:6px 8px;font-size:9px;font-weight:850}
      #algorithmTeacherMonitor .alg-late-v41 select:disabled{opacity:.65;background:#f5f3ed}
      #algorithmTeacherMonitor .alg-late-v41-penalty{min-height:34px;padding:7px 10px;border:1px solid #efb79f;border-radius:9px;background:#fff2ec;color:#b64c25;font-size:8px;font-weight:950;cursor:pointer;white-space:nowrap}
      #algorithmTeacherMonitor .alg-late-v41-penalty.waived{border-color:#b9ddca;background:#edf9f3;color:#247957}
      #algorithmTeacherMonitor .alg-late-v41-foot{margin-top:10px;color:#87734a;font-size:8px;line-height:1.5}
      @media(max-width:900px){#algorithmTeacherMonitor .alg-late-v41-row{grid-template-columns:1fr 1fr}}
      @media(max-width:560px){#algorithmTeacherMonitor .alg-late-v41-head{display:grid}#algorithmTeacherMonitor .alg-late-v41-row{grid-template-columns:1fr}}
    `;
  ...
  function renderPanel(){
    ...
    if(!STATE.manager?.started){
      document.getElementById("algLateJoinPanelV41")?.remove();
      return;
    }

    const records=Object.entries(allLateRecords())
      .map(([key,item])=>({key,...item}))
      .filter(item=>item&&item.name)
      .sort((a,b)=>Number(a.joinedAt||0)-Number(b.joinedAt||0));
    const waiting=records.filter(item=>item.status==="waiting_next_round").length;
    const currentTeams=teams();
    const signature=JSON.stringify(records.map(item=>[item.key,item.status,item.assignedTeam,item.effectiveRoundIndex,item.penaltyWaived,currentTeams.map(team=>team.length)]));

    let panel=document.getElementById("algLateJoinPanelV41");
    if(!panel){
      panel=document.createElement("section");
      panel.id="algLateJoinPanelV41";
      panel.className="alg-late-v41";
      const control=host.querySelector(".alg-tm-control");
    ...
    panel.innerHTML=`
      <div class="alg-late-v41-head">
        <div class="alg-late-v41-title">
          <span class="alg-late-v41-icon">◷</span>
          <div><h4>Siswa terlambat</h4><p>Siswa baru menunggu ronde berikutnya. Ronde aktif tidak berubah dan penalti hanya diterapkan satu kali.</p></div>
        </div>
        <span class="alg-late-v41-count">${waiting} menunggu · ${records.length} tercatat</span>
      </div>
      ${records.length?`<div class="alg-late-v41-list">${records.map(item=>{
        const active=item.status==="active";
        const effective=Number.isFinite(Number(item.effectiveRoundIndex))?Number(item.effectiveRoundIndex)+1:null;
        const options=Array.from({length:teamCount()},(_,i)=>{
          const no=i+1;
          const queued=Object.entries(STATE.manager?.lateQueue||{}).filter(([key,x])=>key!==item.key&&x?.status==="waiting_next_round"&&Number(x.assignedTeam)===no).length;
          const load=(currentTeams[i]?.length||0)+queued;
          const full=load>=TEAM_CAPACITY&&Number(item.assignedTeam)!==no;
          return `<option value="${no}" ${Number(item.assignedTeam)===no?"selected":""} ${full?"disabled":""}>Tim ${no} · ${load}/${TEAM_CAPACITY}</option>`;
        }).join("");
        return `<div class="alg-late-v41-row">
          <div class="alg-late-v41-student"><b>${esc(item.name)}</b><small>${active?`Aktif mulai Algoritma #${effective||"—"}`:(item.assignedTeam?`Masuk pada Algoritma #${effective||"berikutnya"}`:"Belum mendapat tim")}</small></div>
          <span class="alg-late-v41-status ${active?"active":""}">${active?"● Sudah bergabung":"◷ Menunggu ronde"}</span>
          <select data-late-team-v41="${item.key}" ${active?"disabled":""} aria-label="Tim untuk ${esc(item.name)}">
            ${item.assignedTeam?"":'<option value="" selected>Pilih tim</option>'}${options}
          </select>
          <button type="button" class="alg-late-v41-penalty ${item.penaltyWaived?"waived":""}" data-late-penalty-v41="${item.key}">${item.penaltyWaived?"✓ Penalti dibebaskan":"−10 poin aktif"}</button>
        </div>`;
      }).join("")}</div>`:'<div class="alg-late-v41-empty">Belum ada siswa terlambat. Siswa yang sudah tercatat dalam tim dapat masuk kembali tanpa penalti.</div>'}
      <div class="alg-late-v41-foot">Saat anggota tim berubah dari 4 menjadi 5, ronde berikutnya otomatis memakai 20 langkah sehingga setiap siswa tetap memperoleh 4 kartu.</div>
    `;
  }

  async function changeLateTeam(key,teamNo){
    const item=STATE.manager?.lateQueue?.[key];
    if(!item||item.status!=="waiting_next_round"||!STATE.managerRef)return;
    const selected=Math.max(1,Math.min(teamCount(),Number(teamNo)||0));
    if(!selected)return;

    const currentTeams=teams();
    const queued=Object.entries(STATE.manager?.lateQueue||{})
      .filter(([other,x])=>other!==key&&x?.status==="waiting_next_round"&&Number(x.assignedTeam)===selected).length;
    if((currentTeams[selected-1]?.length||0)+queued>=TEAM_CAPACITY){
      alert(`Tim ${selected} sudah mencapai kapasitas ${TEAM_CAPACITY} siswa.`);
      renderPanel();
      return;
    }

    await STATE.managerRef.child(`lateQueue/${key}`).update({
      assignedTeam:selected,
      effectiveRoundIndex:currentRoundIndex(selected)+1,
      reassignedAt:Date.now()
    });
  }

  async function togglePenalty(key){
    if(!STATE.managerRef)return;
    const item=STATE.manager?.lateMembers?.[key]||STATE.manager?.lateQueue?.[key];
    if(!item)return;
    const penaltyWaived=!item.penaltyWaived;
    const updates={
      [`lateQueue/${key}/penalty`]:10,
      [`lateQueue/${key}/penaltyWaived`]:penaltyWaived,
      [`lateQueue/${key}/penaltyUpdatedAt`]:Date.now()
    };
    if(STATE.manager?.lateMembers?.[key]){
      updates[`lateMembers/${key}/penalty`]=10;
      updates[`lateMembers/${key}/penaltyWaived`]=penaltyWaived;
      updates[`lateMembers/${key}/penaltyUpdatedAt`]=Date.now();
    }
    await STATE.managerRef.update(updates);
  }

  function detach(){
    try{STATE.managerRef?.off()}catch(_){}
    try{STATE.lobbyRef?.off()}catch(_){}
    STATE.groupRefs.forEach(ref=>{try{ref.off()}catch(_){}});
    STATE.managerRef=null;STATE.lobbyRef=null;STATE.groupRefs=[];STATE.groups={};
  }

  function attach(){
    if(!teacherActive()||!window.firebase?.database)return;
    const cls=selectedClass();
    if(STATE.cls===cls&&STATE.managerRef&&STATE.lobbyRef)return;

    detach();
    STATE.cls=cls;STATE.manager={};STATE.lobby={};STATE.activeSignature="";
    STATE.managerRef=firebase.database().ref(managerPath(cls));
    STATE.lobbyRef=firebase.database().ref(`kelasExcel/algorithmTeamV3/${safeId(cls)}/${safeId(sessionId())}/lobbyV11`);

    STATE.managerRef.on("value",snap=>{
      STATE.manager=snap.val()||{};
      renderPanel();
      reconcileLateStudents();
    });
    STATE.lobbyRef.on("value",snap=>{
      STATE.lobby=snap.val()||{};
      const signature=activeStudents().join("\u001f");

      // Heartbeat hanya mengubah lastSeen. Jika daftar siswa aktif sama,
      // tidak perlu menjalankan rekonsiliasi/render lagi.
    ...
    for(let no=1;no<=5;no++){
      const ref=firebase.database().ref(groupPath(cls,no));
      STATE.groupRefs.push(ref);
      ref.on("value",snap=>{
        STATE.groups[no]=snap.val()||{};
        renderPanel();
      });
    }
  }
  ...
  function tick(){
    ...
    if(teacherActive()){
      ...
      reconcileLateStudents();
      renderPanel();
    }else if(STATE.managerRef||STATE.lobbyRef){
      detach();STATE.cls="";STATE.manager={};STATE.lobby={};
    }
  }

  document.addEventListener("change",e=>{
    const select=e.target.closest?.("[data-late-team-v41]");
    if(!select)return;
    changeLateTeam(select.dataset.lateTeamV41,select.value).catch(err=>{
      console.error("[V41] Gagal mengganti tim:",err);
      alert("Gagal mengganti tim siswa terlambat. Periksa koneksi Firebase.");
    });
  });

  document.addEventListener("click",e=>{
    const button=e.target.closest?.("[data-late-penalty-v41]");
    if(!button)return;
    togglePenalty(button.dataset.latePenaltyV41).catch(err=>{
      console.error("[V41] Gagal mengubah penalti:",err);
      alert("Gagal mengubah penalti. Periksa koneksi Firebase.");
    });
  });
  ...
  function init(){
    ...
    // Event-driven: jangan mengamati seluruh subtree dashboard dan jangan
    // polling tiap 1,2 detik. Firebase listeners sudah mengirim perubahan data.
    const teacherView=document.getElementById("teacherView");
    if(teacherView){
      const observer=new MutationObserver(()=>setTimeout(tick,0));
      observer.observe(teacherView,{attributes:true,attributeFilter:["class"]});
    }

    window.addEventListener("algorithm-teacher-monitor-rendered",()=>setTimeout(tick,0));

    document.addEventListener("click",e=>{
      if(
        e.target.closest?.("[data-teacher-class]") ||
        e.target.closest?.("#teacherLoginBtn") ||
        e.target.closest?.('[data-preview-lock="6"]')
      ){
        setTimeout(tick,120);
      }
    },true);
    ...
    tick();
    window.AlgorithmLateJoinV41={version:VERSION,refresh:tick};
  }
...
(() => {
  ...
  const HELPERS = RAW`
  /* ===== V41 Algorithm Race + Late Join ===== */

  function algorithmPanelActive(){
    const panel=GAME.panel;
    if(!panel||!panel.isConnected||panel.classList.contains("hidden"))return false;
    if(panel.querySelector(".alg-team-shell,.alg-race-wait-shell"))return true;
    const title=String(panel.querySelector(".l2-window-header h3")?.textContent||"").trim();
    return title==="Algoritma"||
      title==="Algoritma Kelompok"||
      title==="Ruang Tunggu Algoritma"||
      title==="Algorithm Race";
  }

  function cleanupAlgorithmRuntime(){
    if(algorithmPanelActive())return false;
    if(GAME.raceTicker){
      clearInterval(GAME.raceTicker);
      GAME.raceTicker=null;
    }
    if(GAME.raceManagerRef){
      try{GAME.raceManagerRef.off()}catch(_){}
      GAME.raceManagerRef=null;
    }
  ...
  function raceManagerTeamInfo(){
    ...
    const studentKey=safeId(GAME.student?.name);
    const late=data.lateMembers?.[studentKey]||data.lateQueue?.[studentKey]||null;
    const waitingTeam=Math.max(0,Number(late?.assignedTeam)||0);
    return {
      count,teams,assigned:gi>=0,groupIndex:gi,groupNo:gi>=0?gi+1:0,
      members:gi>=0?teams[gi]:[],late,
      waitingTeam,
      waitingMembers:waitingTeam?teams[waitingTeam-1]||[]:[]
    };
  }

  function raceLatePenaltyFor(name=GAME.student?.name){
    const key=safeId(name);
    const meta=GAME.raceManager?.lateMembers?.[key]||GAME.raceManager?.lateQueue?.[key]||null;
    const penalty=meta&&!meta.penaltyWaived?Math.max(0,Number(meta.penalty||10)):0;
    return {meta,penalty};
  }

  async function registerLateStudent(){
    if(GAME.demo||!GAME.raceManager?.started||raceIsExpired()||!window.firebase?.database||!GAME.student?.name)return null;
    const info=raceManagerTeamInfo();
    if(info.assigned)return null;

    const studentKey=safeId(GAME.student.name);
    const known=GAME.raceManager?.lateMembers?.[studentKey]||GAME.raceManager?.lateQueue?.[studentKey];
    if(known)return known;

    const classKey=safeId(GAME.student.className);
    const sessionKey=safeId(currentSessionId());
    const roundIndexes=await Promise.all(info.teams.map(async(_,i)=>{
      try{
        const snap=await firebase.database().ref(
          "kelasExcel/algorithmTeamV3/"+classKey+"/"+sessionKey+"/group_"+(i+1)+"/race/roundIndex"
        ).once("value");
        return Math.max(0,Number(snap.val()||0));
      }catch(_){return 0}
    }));

    const ref=firebase.database().ref("kelasExcel/algorithmManagerV10/"+classKey+"/"+sessionKey);
    let record=null;
    try{
      const tx=await ref.transaction(current=>{
        ...
        if(teams.some(team=>team.includes(GAME.student.name)))return data;
        data.lateQueue=data.lateQueue||{};
        data.lateMembers=data.lateMembers||{};
        if(data.lateQueue[studentKey]||data.lateMembers[studentKey])return data;

        const waiting=Object.values(data.lateQueue).filter(x=>x&&x.status==="waiting_next_round");
        const loads=teams.map((members,i)=>members.length+waiting.filter(x=>Number(x.assignedTeam)===i+1).length);
        let teamIndex=-1;
        loads.forEach((load,i)=>{
          if(load>=6)return;
          if(teamIndex<0||load<loads[teamIndex])teamIndex=i;
        ...
        record={
          name:GAME.student.name,className:GAME.student.className,joinedAt:Date.now(),
          status:"waiting_next_round",assignedTeam:teamIndex+1,
          effectiveRoundIndex:teamIndex>=0?Number(roundIndexes[teamIndex]||0)+1:null,
          penalty:10,penaltyWaived:false,reason:"late_after_start"
        };
        data.lateQueue[studentKey]=record;
        return data;
      });
      const savedManager=tx.snapshot?.val()||GAME.raceManager||{};
      GAME.raceManager=savedManager;
      return savedManager.lateQueue?.[studentKey]||savedManager.lateMembers?.[studentKey]||record;
    }catch(err){
      console.warn("Late join registration:",err);
      return record;
    }
  }

  function renderRaceWaitingRoom(panel){
    if(!panel||!panel.isConnected||!algorithmPanelActive())return;
    ...
    const expired=started&&Number(data.startedAt)>0&&Date.now()>=Number(data.startedAt)+Math.max(1,Number(data.duration||60))*60000;
    const header=panel.querySelector(".l2-window-header");

    const lateWaiting=started&&!info.assigned;
    const targetTeam=lateWaiting?info.waitingTeam:info.groupNo;
    const displayMembers=lateWaiting?info.waitingMembers:info.members;
    const effectiveRound=Number(info.late?.effectiveRoundIndex);
    const latePenaltyWaived=!!info.late?.penaltyWaived;
    const title=lateWaiting&&expired
      ?"Permainan telah berakhir"
      :lateWaiting
      ?(targetTeam?\`Menunggu ronde \${Number.isFinite(effectiveRound)?effectiveRound+1:"berikutnya"}\`:"Menunggu penempatan guru")
      :(info.assigned?\`Anda masuk Tim \${info.groupNo}\`:"Menunggu pembagian tim");
    let desc="";
    let stateClass="wait";
    let icon="👥";
    ...
    if(lateWaiting){
      desc=expired
        ?"Waktu permainan sudah habis sebelum Anda sempat masuk ke ronde. Hubungi guru untuk tindak lanjut nilai."
        :(targetTeam
          ?\`Permainan sudah berjalan. Anda akan masuk Tim \${targetTeam} saat tim berpindah ke ronde berikutnya. \${latePenaltyWaived?"Penalti telah dibebaskan guru.":"Nilai akhir mendapat penalti 10 poin."}\`
          :"Permainan sudah berjalan dan seluruh tim yang tersedia sudah penuh. Tunggu guru menentukan tim untuk ronde berikutnya.");
      stateClass="late";
      icon=expired?"■":"◷";
    }else if(!info.assigned){
    ...
    const members=displayMembers.length
      ...
      : \`<div class="alg-race-wait-empty">\${lateWaiting?"Tim akan ditentukan guru.":"Anggota tim akan muncul setelah guru membagi siswa."}</div>\`;

    const body=document.createElement("div");
    body.className="alg-race-wait-shell";
    body.innerHTML=\`
      <section class="alg-race-wait-card">
        <div class="alg-race-wait-visual" aria-hidden="true">
          <span class="alg-race-visual-badge">ALGORITHM RACE</span>
          <div class="alg-race-visual-orbit orbit-one"></div>
          <div class="alg-race-visual-orbit orbit-two"></div>
          <img src="assets/student-mascot-3d-v2.png" alt="" />
          <div class="alg-race-visual-copy">
            <strong>Siap menyusun langkah?</strong>
            <small>Diskusikan, urutkan, lalu taklukkan tantangannya bersama tim.</small>
          </div>
          <span class="alg-race-visual-leaf leaf-a"></span>
          <span class="alg-race-visual-leaf leaf-b"></span>
        </div>

        <div class="alg-race-wait-content">
          <div class="alg-race-wait-title-row">
          ...
          <p class="alg-race-wait-description">\${esc(desc)}</p>

          <div class="alg-race-wait-status">
            <div><span class="alg-race-status-icon">●</span><small>Status Anda</small><strong>\${lateWaiting?(targetTeam?\`Antre Tim \${targetTeam}\`:"Menunggu guru"):(info.assigned?\`Tim \${info.groupNo}\`:"Sudah masuk halaman")}</strong></div>
            <div><span class="alg-race-status-icon">⌘</span><small>Jumlah tim</small><strong>\${data.teamCount||"—"}</strong></div>
            <div><span class="alg-race-status-icon">◷</span><small>Durasi</small><strong>\${data.duration?\`\${data.duration} menit\`:"Belum dipilih"}</strong></div>
            <div><span class="alg-race-status-icon">▶</span><small>Permainan</small><strong>\${lateWaiting?(expired?"Waktu habis":(latePenaltyWaived?"Berjalan · tanpa penalti":"Berjalan · −10 poin")):(started?"Sedang dimulai":"Belum dimulai")}</strong></div>
          </div>

          \${targetTeam?\`
            <div class="alg-race-wait-team">
              <div class="alg-race-wait-team-head">
                <b>\${lateWaiting?"Calon anggota":"Anggota"} Tim \${targetTeam}</b><span>\${displayMembers.length} siswa\${lateWaiting?" saat ini":""}</span>
              </div>
          ...
          <div class="alg-race-wait-notice">
            <span>◷</span>
            <div>
              <b>\${lateWaiting?(expired?"Sesi ini sudah ditutup.":"Anda akan masuk otomatis pada ronde berikutnya."):"Halaman ini akan berubah otomatis."}</b>
              <p>\${lateWaiting?(expired?"Hubungi guru untuk tindak lanjut. Memuat ulang halaman tidak akan membuka kembali waktu permainan.":"Ronde yang sedang berjalan tidak berubah. Saat ronde baru dimulai, jumlah langkah akan menyesuaikan anggota tim dan kartu dibagikan ulang secara merata."):"Tidak perlu memuat ulang. Setelah guru membagi tim dan menekan Mulai, papan Algorithm Race akan langsung terbuka."}</p>
            </div>
          </div>
        </div>
      </section>\`;
  ...
  function enterRaceGameFromWaiting(){
    if(!GAME.panel||!GAME.panel.isConnected||!algorithmPanelActive())return;
    GAME.raceWaiting=false;
    GAME.selected=null;
    GAME.completed=false;
    GAME.chat=GAME.chat||[];
  ...
  function raceTaskInfoHtml(){
    ...
    const late=raceLatePenaltyFor();
    const lateNotice=late.meta
      ?\`<div class="alg-late-student-note \${late.penalty?"":"waived"}"><b>◷ Siswa terlambat</b><span>Mulai ikut dari Algoritma #\${Math.max(1,Number(late.meta.effectiveRoundIndex||0)+1)} · \${late.penalty?\`penalti −\${late.penalty} poin\`:"penalti dibebaskan guru"}</span></div>\`
      :"";
    ...
    return \`\${lateNotice}<div class="alg-task-info">
  ...
  function updateAlgorithmHeaderTime(){
    ...
    const progressTimer=panel.querySelector("#exerciseWindowTimer");
    if(progressTimer)progressTimer.textContent=timeText;
  }
  ...
  function raceIndividualMetrics(name=GAME.student?.name){
    ...
    const late=raceLatePenaltyFor(name);
    const effectiveRoundIndex=Math.max(0,Number(late.meta?.effectiveRoundIndex)||0);
    const eligibleCompleted=Math.max(0,team.completed-effectiveRoundIndex);
    const startedRound=(Number(stats.placements||0)+Number(stats.revisions||0)+Number(stats.tests||0)+chatCount)>0?1:0;
    const rounds=Math.max(1,eligibleCompleted+startedRound);
    ...
    const baseFinalScore=Math.max(0,Math.min(100,Math.round(team.teamScore*.70+contribution*.30)));
    const finalScore=Math.max(0,baseFinalScore-late.penalty);

    return {
      stats,chatCount,cardsPerRound,placementTarget,revisionTarget,testTarget,chatTarget,
      placementScore,revisionScore,testingScore,discussionScore,contribution,
      effectiveRoundIndex,eligibleCompleted,baseFinalScore,latePenalty:late.penalty,lateMeta:late.meta,finalScore,
      ...team
    };
  }
  ...
  function applyRaceRound(roundIndex,quiet=false){
    ...
    const rawRoundMembers=GAME.meta?.race?.roundMembers;
    const roundMembers=Array.isArray(rawRoundMembers)
      ?rawRoundMembers.filter(Boolean)
      :(rawRoundMembers&&typeof rawRoundMembers==="object"?Object.values(rawRoundMembers).filter(Boolean):[]);
    if(roundMembers.length&&roundMembers.includes(GAME.student?.name)){
      GAME.members=roundMembers;
      GAME.slot=Math.max(0,GAME.members.indexOf(GAME.student.name));
    }
    GAME.raceRound=next;
    ...
    try{
      await ref.transaction(cur=>{
        ...
        return {
          ...
          roundIndex:0,
          roundMembers:[...(GAME.members||[])],
          roundStepCount:GAME.steps.length,
  ...
  async function completeRaceAlgorithm(){
    ...
    if(Number(race.roundIndex||0)!==round){
    ...
    let managerData=GAME.raceManager||{};
    try{
      const managerSnap=await (GAME.raceManagerRef||firebase.database().ref(
        \`kelasExcel/algorithmManagerV10/\${safeId(GAME.student.className)}/\${safeId(currentSessionId())}\`
      )).once("value");
      managerData=managerSnap.val()||managerData;
    }catch(_){}

    const teamCount=[3,4,5].includes(Number(managerData.teamCount))?Number(managerData.teamCount):5;
    const rawTeams=managerData.teams||{};
    const managerTeams=Array.from({length:teamCount},(_,i)=>{
      const v=rawTeams[i]??rawTeams[String(i)]??rawTeams[i+1]??rawTeams[String(i+1)];
      if(Array.isArray(v))return v.filter(Boolean);
      if(v&&typeof v==="object")return Object.values(v).filter(Boolean);
      return [];
    });
    const currentMembers=managerTeams[GAME.groupIndex]?.length?managerTeams[GAME.groupIndex]:GAME.members;
    const lateCandidates=Object.entries(managerData.lateQueue||{})
      .filter(([,entry])=>entry&&entry.status==="waiting_next_round"&&Number(entry.assignedTeam)===GAME.groupNo)
      .filter(([,entry])=>!Number.isFinite(Number(entry.effectiveRoundIndex))||Number(entry.effectiveRoundIndex)<=nextRound)
      .sort((a,b)=>Number(a[1].joinedAt||0)-Number(b[1].joinedAt||0));
    const capacity=Math.max(0,6-currentMembers.length);
    const activatedLate=lateCandidates.slice(0,capacity);
    const nextMembers=[...new Set([...currentMembers,...activatedLate.map(([,entry])=>entry.name).filter(Boolean)])];
    const nextStepCount=stepsForGroup(nextScenario,nextMembers.length).length;
    ...
    const completion={
      ...
      activatedLateStudents:activatedLate.map(([,entry])=>entry.name).filter(Boolean)
    };
    ...
    updates["race/roundMembers"]=nextMembers;
    updates["race/roundStepCount"]=nextStepCount;
    ...
    updates["groupSize"]=nextMembers.length;
    updates["stepCount"]=nextStepCount;

    const classKey=safeId(GAME.student.className);
    const sessionKey=safeId(currentSessionId());
    const groupBase=\`algorithmTeamV3/\${classKey}/\${sessionKey}/group_\${GAME.groupNo}\`;
    const managerBase=\`algorithmManagerV10/\${classKey}/\${sessionKey}\`;
    const rootUpdates={};
    Object.entries(updates).forEach(([path,value])=>{rootUpdates[\`\${groupBase}/\${path}\`]=value});

    if(activatedLate.length){
      managerTeams[GAME.groupIndex]=nextMembers;
      rootUpdates[\`\${managerBase}/teams/\${GAME.groupIndex}\`]=nextMembers;
      rootUpdates[\`\${managerBase}/assignedNames\`]=[...new Set(managerTeams.flat().filter(Boolean))];
      rootUpdates[\`\${managerBase}/lateActivatedAt\`]=now;
      for(const [key,entry] of activatedLate){
        const activeRecord={
          ...entry,
          status:"active",
          assignedTeam:GAME.groupNo,
          effectiveRoundIndex:nextRound,
          activatedAt:now,
          penalty:Math.max(0,Number(entry.penalty||10)),
          penaltyWaived:!!entry.penaltyWaived
        };
        rootUpdates[\`\${managerBase}/lateQueue/\${key}\`]=activeRecord;
        rootUpdates[\`\${managerBase}/lateMembers/\${key}\`]=activeRecord;
      }
    }

    try{
      await firebase.database().ref("kelasExcel").update(rootUpdates);
    }catch(err){
      try{await claimRef.remove()}catch(_){}
      GAME.lastHint="Perpindahan ronde gagal disimpan. Periksa koneksi lalu tekan Uji kembali.";
      console.warn("Race round transition:",err);
      render();
      return false;
    }

    const activatedText=activatedLate.length
      ?\` \${activatedLate.map(([,entry])=>entry.name).join(", ")} bergabung; ronde baru memakai \${nextMembers.length} anggota dan \${nextStepCount} langkah.\`
      :"";
    await GAME.rootRef.child("chat").push().set({
      name:"Sistem",
      system:true,
      at:now,
      text:\`✅ Algoritma \${round+1} benar. Tim mendapat \${earned} poin (50 dasar + \${bonus} bonus waktu). Algoritma \${nextRound+1} dibuka.\${activatedText}\`
    });
  ...
  function updateRaceClockDom(){
    if(!algorithmPanelActive())return;
  ...
  async function finalizeRaceStudentScore(){
    ...
    if(!raceManagerTeamInfo().assigned)return;
    ...
    try{
      ...
      rec.exerciseState[6]={
        ...
        teamAlgorithmRace:{
          ...
          baseFinalScore:metrics.baseFinalScore,
          latePenalty:metrics.latePenalty,
          lateJoin:!!metrics.lateMeta,
          effectiveRoundIndex:metrics.effectiveRoundIndex,
      ...
      const scoreDetail=metrics.latePenalty
        ?\` Nilai dasar \${metrics.baseFinalScore} dikurangi penalti keterlambatan \${metrics.latePenalty} poin.\`
        :(metrics.lateMeta?" Penalti keterlambatan telah dibebaskan guru.":"");
      GAME.lastHint=\`⏱ Waktu habis. Nilai akhir Algoritma \${metrics.finalScore}/100 tersimpan.\${scoreDetail} Tim menyelesaikan \${metrics.completed} algoritma.\`;
      render();
    }finally{
  ...
  function startRaceTicker(){
    ...
    GAME.raceTicker=setInterval(async()=>{
      if(!algorithmPanelActive()){
        cleanupAlgorithmRuntime();
        return;
      }
      updateRaceClockDom();
  ...
  function listenRaceManager(){
    ...
    GAME.raceManagerRef.on("value",async snap=>{
      ...
      if(!raceManagerTeamInfo().assigned){
        await registerLateStudent();
        if(GAME.panel?.isConnected)renderRaceWaitingRoom(GAME.panel);
        updateRaceClockDom();
        return;
      }
...
`;
  ...
  function load(){
    try{
      ...
      // Jangan izinkan listener Algorithm Race menggambar ulang latihan lain
      // setelah siswa berpindah dari Latihan 7.
      src=replaceRequired(
        src,
        '  function render(){\n    const panel=GAME.panel;if(!panel||!panel.isConnected)return;',
        '  function render(){\n    const panel=GAME.panel;if(!panel||!panel.isConnected||!algorithmPanelActive())return;',
        "guard render hanya pada Exercise 7"
      );
      ...
      // Read the active round snapshot before assigning cards. This prevents a
      // newly activated late student from briefly receiving cards from the old round.
      const oldAssignment=RAW`      GAME.groupIndex=gi;
      GAME.groupNo=gi+1;
      GAME.members=teams[gi];
      GAME.slot=Math.max(0,GAME.members.indexOf(GAME.student.name));
      GAME.scenario=raceScenarioForRound(GAME.student.className,GAME.groupIndex,Number(GAME.raceRound||0));`;
      const newAssignment=RAW`      GAME.groupIndex=gi;
      GAME.groupNo=gi+1;
      let groupData={};
      try{
        const groupSnap=await firebase.database().ref(
          \`kelasExcel/algorithmTeamV3/\${safeId(GAME.student.className)}/\${safeId(currentSessionId())}/group_\${GAME.groupNo}\`
        ).once("value");
        groupData=groupSnap.val()||{};
      }catch(_){}
      GAME.meta=groupData;
      GAME.raceRound=Math.max(0,Number(groupData.race?.roundIndex||0));
      const rawRoundMembers=groupData.race?.roundMembers;
      const roundMembers=Array.isArray(rawRoundMembers)
        ?rawRoundMembers.filter(Boolean)
        :(rawRoundMembers&&typeof rawRoundMembers==="object"?Object.values(rawRoundMembers).filter(Boolean):[]);
      GAME.members=roundMembers.includes(GAME.student.name)?roundMembers:teams[gi];
      GAME.slot=Math.max(0,GAME.members.indexOf(GAME.student.name));
      GAME.scenario=raceScenarioForRound(GAME.student.className,GAME.groupIndex,Number(GAME.raceRound||0));`;
      src=replaceRequired(src,oldAssignment,newAssignment,"snapshot ronde untuk anggota terlambat");
      ...
      src=replaceRequired(
        src,
        '    render();connect();\n    panel.dataset.algMounting="";',
        '    listenRaceManager();startRaceTicker();\n    if(!GAME.demo && GAME.raceManager?.started && !raceManagerTeamInfo().assigned){\n      await registerLateStudent();\n    }\n    if(!GAME.demo && (!GAME.raceManager?.started || !raceManagerTeamInfo().assigned)){\n      renderRaceWaitingRoom(panel);\n      panel.dataset.algMounting="";\n      return;\n    }\n    render();connect();\n    panel.dataset.algMounting="";',
        "mount waiting/race"
      );
    ...
    /* V40 — ruang tunggu menyatu dengan tema hijau pastel */
    .alg-race-wait-shell{
      position:relative;isolation:isolate;overflow:hidden;width:100%;min-height:clamp(560px,72vh,780px);
      padding:clamp(30px,4.2vw,66px);display:grid;place-items:center;
      background:
        radial-gradient(circle at 5% 18%,rgba(40,200,140,.20),transparent 25%),
        radial-gradient(circle at 96% 82%,rgba(76,190,213,.17),transparent 27%),
        linear-gradient(145deg,#f8fffb 0%,#effaf5 47%,#f5fbff 100%);
    }
    .alg-race-wait-shell:before,.alg-race-wait-shell:after{
      content:"";position:absolute;z-index:-1;border-radius:46% 54% 62% 38%;pointer-events:none;
    }
    .alg-race-wait-shell:before{
      width:360px;height:260px;left:-155px;bottom:-85px;background:rgba(35,196,134,.10);transform:rotate(23deg);
    }
    .alg-race-wait-shell:after{
      width:330px;height:235px;right:-145px;top:-65px;background:rgba(75,181,217,.10);transform:rotate(-18deg);
    }
    .alg-race-wait-card{
      position:relative;overflow:hidden;width:min(1280px,100%);display:grid;
      grid-template-columns:minmax(350px,.82fr) minmax(520px,1.18fr);
      background:#fff;border:1px solid #d2e8de;border-radius:32px;padding:0;
      box-shadow:0 26px 70px rgba(5,75,54,.15);text-align:left;
      animation:algRaceWaitEnter .55s cubic-bezier(.2,.8,.2,1) both;
    }
    .alg-race-wait-visual{
      position:relative;isolation:isolate;overflow:hidden;min-height:530px;padding:30px;
      display:flex;align-items:flex-end;justify-content:center;
      background:
        radial-gradient(circle at 52% 43%,rgba(83,239,179,.27),transparent 33%),
        radial-gradient(circle at 15% 92%,rgba(255,213,73,.14),transparent 26%),
        linear-gradient(145deg,#087558 0%,#075d49 54%,#054638 100%);
    }
    .alg-race-wait-visual:after{
      content:"";position:absolute;z-index:-1;left:-14%;right:-14%;bottom:-105px;height:230px;
      border-radius:50%;background:rgba(85,224,166,.12);
    }
    .alg-race-visual-badge{
      position:absolute;left:28px;top:27px;z-index:4;padding:9px 14px;border:1px solid rgba(255,255,255,.23);
      border-radius:999px;background:rgba(255,255,255,.10);color:#d8fff0;font-size:10px;font-weight:950;letter-spacing:.13em;
    }
    .alg-race-wait-visual img{
      position:absolute;z-index:2;left:50%;top:47%;width:min(540px,126%);max-width:none;height:auto;
      transform:translate(-50%,-50%);filter:drop-shadow(0 20px 22px rgba(0,36,25,.25));
      animation:algRaceMascotFloat 5s ease-in-out infinite;
    }
    .alg-race-visual-copy{
      position:relative;z-index:4;width:100%;padding:20px 22px;border:1px solid rgba(255,255,255,.20);
      border-radius:20px;background:rgba(3,58,43,.62);box-shadow:0 14px 30px rgba(0,34,24,.18);
      backdrop-filter:blur(10px);color:#fff;
    }
    .alg-race-visual-copy strong{display:block;font-size:22px;line-height:1.2}
    .alg-race-visual-copy small{display:block;margin-top:7px;color:#cceadf;font-size:12px;line-height:1.5}
    .alg-race-visual-orbit{position:absolute;z-index:1;border:1px solid rgba(164,255,217,.23);border-radius:50%}
    .alg-race-visual-orbit.orbit-one{width:305px;height:305px;left:50%;top:44%;transform:translate(-50%,-50%)}
    .alg-race-visual-orbit.orbit-two{width:410px;height:410px;left:50%;top:44%;transform:translate(-50%,-50%)}
    .alg-race-visual-leaf{position:absolute;z-index:3;width:27px;height:16px;border-radius:100% 0 100% 0;background:#65dba3;opacity:.7}
    .alg-race-visual-leaf.leaf-a{right:28px;top:88px;transform:rotate(-35deg);animation:algRaceLeaf 6s ease-in-out infinite}
    .alg-race-visual-leaf.leaf-b{left:32px;top:155px;transform:rotate(32deg);animation:algRaceLeaf 7s ease-in-out -2.5s infinite}
    .alg-race-wait-content{
      min-width:0;padding:clamp(34px,4vw,58px);align-self:center;
      background:
        radial-gradient(circle at 104% -5%,rgba(64,216,154,.10),transparent 29%),
        radial-gradient(circle at -4% 105%,rgba(255,210,70,.08),transparent 24%),#fff;
    }
    .alg-race-wait-title-row{display:grid;grid-template-columns:66px minmax(0,1fr);gap:17px;align-items:center}
    .alg-race-wait-title-row>div:last-child{min-width:0}
    .alg-race-wait-icon{
      width:66px;height:66px;margin:0;display:grid;place-items:center;border-radius:20px;
      background:linear-gradient(145deg,#e2fff1,#bdf1d7);color:#087a56;font-size:29px;
      box-shadow:0 10px 22px rgba(7,137,92,.12);animation:algRaceStatusPulse 2.8s ease-in-out infinite;
    }
    .alg-race-wait-icon.ready{background:linear-gradient(145deg,#dffff0,#a9eccb);color:#087851}
    .alg-race-wait-icon.late{background:linear-gradient(145deg,#fff6d7,#ffe6a3);color:#ad7200}
    .alg-race-wait-kicker{display:block;color:#07966a;font-size:10px;font-weight:950;letter-spacing:.12em}
    .alg-race-wait-card h3{margin:7px 0 0;color:#0b2639;font-size:clamp(27px,2.25vw,38px);line-height:1.08;font-weight:950;letter-spacing:-.035em}
    .alg-race-wait-description{width:min(650px,100%);margin:20px 0 0;color:#607789;font-size:14px;line-height:1.68}
    .alg-race-wait-status{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;margin-top:26px}
    .alg-race-wait-status>div{
      position:relative;display:grid;grid-template-columns:38px minmax(0,1fr);grid-template-areas:"icon label" "icon value";
      column-gap:11px;padding:15px 16px;border:1px solid #dcebe5;border-radius:17px;
      background:linear-gradient(145deg,#fbfffd,#f2faf6);text-align:left;box-shadow:0 5px 13px rgba(13,87,63,.045);
    }
    .alg-race-status-icon{
      grid-area:icon;align-self:center;width:38px;height:38px;display:grid;place-items:center;border-radius:12px;
      background:#ddf8eb;color:#0b9b6b;font-size:12px;font-weight:950;
    }
    .alg-race-wait-status>div:nth-child(2) .alg-race-status-icon{background:#eaf1ff;color:#3c6fdc}
    .alg-race-wait-status>div:nth-child(3) .alg-race-status-icon{background:#fff4d9;color:#d78c12}
    .alg-race-wait-status>div:nth-child(4) .alg-race-status-icon{background:#f2eaff;color:#8254cc}
    .alg-race-wait-status small{grid-area:label;display:block;color:#789087;font-size:9px;font-weight:850}
    .alg-race-wait-status strong{grid-area:value;display:block;margin-top:3px;color:#17372d;font-size:12px;line-height:1.25}
    .alg-race-wait-team{margin-top:18px;border:1px solid #d8e9e1;border-radius:18px;overflow:hidden;text-align:left;background:#fff}
    .alg-race-wait-team-head{display:flex;justify-content:space-between;gap:12px;align-items:center;padding:13px 16px;background:#f1faf5;border-bottom:1px solid #dcebe4}
    .alg-race-wait-team-head b{color:#1a4b3b;font-size:11px}
    .alg-race-wait-team-head span{padding:5px 9px;border-radius:999px;background:#dff5ea;color:#17805c;font-size:8px;font-weight:900}
    .alg-race-wait-members{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;padding:12px}
    .alg-race-wait-member{display:grid;grid-template-columns:34px 1fr;gap:9px;align-items:center;padding:9px;border:1px solid #e2eee9;border-radius:12px;background:#fbfefd}
    .alg-race-wait-member>span{width:34px;height:34px;display:grid;place-items:center;border-radius:11px;background:#e7f4ee;color:#39705d;font-size:9px;font-weight:950}
    .alg-race-wait-member.me{border-color:#83d9b6;background:#effbf5}
    .alg-race-wait-member.me>span{background:#11a875;color:#fff}
    .alg-race-wait-member b{display:block;color:#254b3e;font-size:10px}
    .alg-race-wait-member small{display:block;margin-top:2px;color:#7f968e;font-size:8px}
    .alg-race-wait-empty{grid-column:1/-1;padding:13px;text-align:center;color:#81958e;font-size:10px}
    .alg-race-wait-notice{display:grid;grid-template-columns:42px 1fr;gap:12px;align-items:center;text-align:left;margin-top:18px;padding:14px 15px;border-radius:16px;background:linear-gradient(135deg,#edf9f3,#f6fcf9);border:1px solid #cde9db}
    .alg-race-wait-notice>span{width:42px;height:42px;display:grid;place-items:center;border-radius:13px;background:#fff;color:#0a9a68;box-shadow:0 5px 12px rgba(8,127,87,.08)}
    .alg-race-wait-notice b{display:block;color:#176348;font-size:10px}
    .alg-race-wait-notice p{margin:4px 0 0;color:#68847a;font-size:9px;line-height:1.5}
    @keyframes algRaceWaitEnter{from{opacity:0;transform:translateY(18px) scale(.985)}to{opacity:1;transform:translateY(0) scale(1)}}
    @keyframes algRaceMascotFloat{0%,100%{translate:0 0}50%{translate:0 -7px}}
    @keyframes algRaceLeaf{0%,100%{translate:0 0;rotate:0deg}50%{translate:5px -10px;rotate:11deg}}
    @keyframes algRaceStatusPulse{0%,100%{transform:scale(1);box-shadow:0 10px 22px rgba(7,137,92,.12)}50%{transform:scale(1.035);box-shadow:0 12px 28px rgba(7,137,92,.20)}}
    @media(max-width:980px){
      .alg-race-wait-card{grid-template-columns:1fr}
      .alg-race-wait-visual{min-height:350px}
      .alg-race-wait-visual img{width:min(510px,86%);top:46%}
      .alg-race-visual-copy{max-width:620px}
    }
    @media(max-width:620px){
      .alg-race-wait-shell{padding:18px 12px 25px}
      .alg-race-wait-card{border-radius:24px}
      .alg-race-wait-visual{min-height:305px;padding:20px}
      .alg-race-visual-badge{left:20px;top:19px}
      .alg-race-wait-visual img{width:390px;max-width:112%;top:47%}
      .alg-race-visual-copy{padding:15px 16px}
      .alg-race-visual-copy strong{font-size:18px}
      .alg-race-wait-content{padding:27px 20px}
      .alg-race-wait-title-row{grid-template-columns:54px minmax(0,1fr);gap:13px}
      .alg-race-wait-icon{width:54px;height:54px;border-radius:17px;font-size:23px}
      .alg-race-wait-status{grid-template-columns:1fr}
      .alg-race-wait-members{grid-template-columns:1fr}
    }
    @media(prefers-reduced-motion:reduce){
      .alg-race-wait-card,.alg-race-wait-icon,.alg-race-wait-visual img,.alg-race-visual-leaf{animation:none!important}
    }

    .alg-late-student-note{
      display:flex;align-items:center;justify-content:space-between;gap:12px;margin-top:14px;padding:10px 12px;
      border:1px solid #f1c49e;border-radius:12px;background:#fff4eb;color:#a64f22;font-size:9px;line-height:1.4;
    }
    .alg-late-student-note b{font-weight:950}.alg-late-student-note span{text-align:right}
    .alg-late-student-note.waived{border-color:#bfe3cf;background:#effaf4;color:#267655}
    .alg-task-info{
      display:grid;
...
(() => {
  ...
  function scheduleTeacherUi(delay=70){
    clearTimeout(STATE.teacherUiTimer);
    STATE.teacherUiTimer=setTimeout(()=>{
      STATE.teacherUiTimer=null;
      applyTeacherUi();
    },delay);
  }
  ...
  function ensureTeacherListeners(){
    ...
    STATE.joined = [];
    STATE.manager = {};
    STATE.joinedSignature = "";
    STATE.managerSignature = "";
    ...
    STATE.teacherPresenceRef.on("value", snap=>{
      ...
      // Heartbeat lastSeen tidak boleh menyebabkan repaint dashboard guru.
      const signature=activeJoinedNames().join("\u001f");
      if(signature===STATE.joinedSignature)return;
      STATE.joinedSignature=signature;
      scheduleTeacherUi();
    });
    ...
    STATE.teacherManagerRef.on("value", snap=>{
      ...
      const signature=JSON.stringify({
        teamCount:next.teamCount||5,
        duration:next.duration||50,
        teams:next.teams||null,
        v11Assigned:!!next.v11Assigned,
        assignedNames:next.assignedNames||null,
        started:!!next.started,
        stopped:!!next.stopped,
        scoreDisabled:!!next.scoreDisabled
      });
      if(signature===STATE.managerSignature)return;
      STATE.managerSignature=signature;
      scheduleTeacherUi();
    });
  }
  ...
  function applyTeacherUi(){
    ...
    try{
      ...
      const joinedText=String(joined.length);
      if(joinedStrong && joinedStrong.textContent!==joinedText){
        joinedStrong.textContent=joinedText;
      }

      const online = $(".alg-tm-online",host);
      if(online){
        const onlineText=joined.length
          ? `${joined.length} siswa berada di halaman Latihan Algoritma`
          : "Menunggu siswa membuka Latihan Algoritma";
        if(online.dataset.v11PresenceMessage!==onlineText){
          online.dataset.v11PresenceMessage=onlineText;
          online.innerHTML=`<i></i>${onlineText}`;
        }
      }
  ...
  function init(){
    ...
    // Observer hanya pada area siswa dan status view. Jangan mengamati subtree
    // dashboard guru karena setiap perubahan kecil akan memicu siklus UI baru.
    const observer=new MutationObserver(()=>{
      clearTimeout(observer._timer);
      observer._timer=setTimeout(tick,50);
    });
    ...
    if(studentView){
      observer.observe(studentView,{attributes:true,attributeFilter:["class"]});
    }
    if(exercisePanel){
      observer.observe(exercisePanel,{
        childList:true,
        subtree:true,
        attributes:true,
        attributeFilter:["class"]
    ...
    window.addEventListener("algorithm-teacher-monitor-rendered",()=>{
      ensureTeacherListeners();
      scheduleTeacherUi(0);
    });
...
(() => {
  ...
  const VERSION="algorithm-race-dashboard-v41";
  const STATE={
    cls:"",
    manager:{},
    groups:{},
  ...
  function lateMetaFor(name){
    const key=safeId(name);
    return STATE.manager?.lateMembers?.[key]||STATE.manager?.lateQueue?.[key]||null;
  }
  ...
  function contributionFor(name,group,members){
    ...
    const lateMeta=lateMetaFor(name);
    const effectiveRoundIndex=Math.max(0,Number(lateMeta?.effectiveRoundIndex)||0);
    const eligibleCompleted=Math.max(0,m.completed-effectiveRoundIndex);
    const attempted=(Number(stats.placements||0)+Number(stats.revisions||0)+Number(stats.tests||0)+chat)>0?1:0;
    const rounds=Math.max(1,eligibleCompleted+attempted);
    const placementTarget=Math.max(1,cardsPerRound*rounds);
    const revisionTarget=Math.max(1,Math.ceil(rounds*.75));
    const testTarget=Math.max(1,Math.ceil(rounds/2));
    const chatTarget=Math.max(2,rounds*2);
    ...
    const baseFinalScore=Math.max(0,Math.min(100,Math.round(m.score*.70+contribution*.30)));
    const latePenalty=lateMeta&&!lateMeta.penaltyWaived?Math.max(0,Number(lateMeta.penalty||10)):0;
    const finalScore=Math.max(0,baseFinalScore-latePenalty);

    return {stats,chat,contribution,baseFinalScore,latePenalty,lateMeta,effectiveRoundIndex,eligibleCompleted,finalScore,...m};
  }
  ...
  function injectCss(){
    ...
    s.textContent=`
      ...
      .alg-race-penalty{display:inline-flex;padding:4px 7px;border-radius:999px;background:#fff0e8;color:#c35428;font-weight:950}
      .alg-race-penalty.waived{background:#ebf8f1;color:#27825a}
      .alg-race-final-note{
  ...
  function listen(){
    ...
    STATE.managerRef.on("value",snap=>{
      ...
      STATE.manager=snap.val()||{};
      const nextCount=teamCount();

      // Jangan lepas/pasang ulang seluruh listener grup untuk perubahan manager
      // yang tidak mengubah jumlah tim.
      if(!STATE.groupRefs.length || previousCount!==nextCount){
        listenGroups();
      }
      render();
    });
  ...
  function render(){
    ...
    panel.innerHTML=`
      ...
      <div class="alg-race-formula">
        <b>Skor kompetisi:</b> 50 poin setiap algoritma benar + bonus waktu 0–4.
        <b>Peringkat:</b> jumlah algoritma selesai → total poin → waktu.
        <br><b>Nilai Tim:</b> 80% progres target + 20% efisiensi bonus.
        <b>Nilai siswa:</b> 70% Nilai Tim + 30% kontribusi individu, lalu penalti keterlambatan dikurangi satu kali.
      </div>
        ...
        <table class="alg-race-table">
          <thead>
            <tr>
              ...
              <th>Poin Tim</th><th>Nilai Tim</th><th>Kontribusi</th><th>Nilai Dasar</th><th>Penalti</th><th>Nilai Akhir</th>
            </tr>
          </thead>
          <tbody>
            ${rows.length?[...rows].sort((a,b)=>b.finalScore-a.finalScore).map((r,i)=>`
              <tr>
                ...
                <td>${r.contribution}</td><td>${r.baseFinalScore}</td>
                <td><span class="alg-race-penalty ${r.lateMeta&&r.latePenalty===0?"waived":""}">${r.lateMeta?(r.latePenalty?`−${r.latePenalty}`:"Dibebaskan"):"—"}</span></td>
                <td><b>${r.finalScore}</b></td>
              </tr>
            `).join(""):`<tr><td colspan="10">Belum ada siswa yang terbagi.</td></tr>`}
          </tbody>
        </table>
      </div>
  ...
  function updateClockOnly(){
    ...
    const started=!!STATE.manager?.started;
    const ended=started&&timeLeft()<=0;
    const clock=panel.querySelector(".alg-race-clock");
    if(clock){
      const label=clock.querySelector("small");
      const value=clock.querySelector("strong");
      if(label)label.textContent=ended?"Waktu habis":"Sisa waktu";
      if(value)value.textContent=started?timeText(timeLeft()):"Belum mulai";
    }

    // Finalisasi tetap dijalankan saat waktu habis, tetapi tabel/leaderboard
    // tidak dibangun ulang setiap detik.
    if(ended)finalizeAllStudents();
  }
  ...
  function init(){
    ...
    const teacherView=document.getElementById("teacherView");
    if(teacherView){
      const obs=new MutationObserver(()=>setTimeout(tick,0));
      obs.observe(teacherView,{attributes:true,attributeFilter:["class"]});
    }

    window.addEventListener("algorithm-teacher-monitor-rendered",()=>{
      if(document.getElementById("teacherView")?.classList.contains("active")){
        listen();
        render();
      }
    });
    ...
    document.addEventListener("click",e=>{
      if(e.target.closest?.("[data-teacher-class]")||e.target.closest?.("#teacherLoginBtn")){
        setTimeout(()=>{
          listen();
          render();
        },120);
    ...
    // Satu-satunya polling yang dipertahankan di dashboard guru:
    // memperbarui angka countdown tanpa membangun ulang tabel.
...
(() => {
  ...
  function init(){
    ...
    const teacherView=document.getElementById("teacherView");
    if(teacherView){
      observer=new MutationObserver(()=>setTimeout(sync,0));
      observer.observe(teacherView,{attributes:true,attributeFilter:["class"]});
    }

    window.addEventListener("algorithm-teacher-monitor-rendered",()=>setTimeout(sync,0));
...
(() => {
  ...
  function updateDurationUi(host){
    ...
    host.querySelectorAll("[data-alg-duration]").forEach(btn=>{
      ...
      if(btn.disabled!==started)btn.disabled=started;
    });
  ...
  function attach(){
    ...
    lobbyRef.on("value",snap=>{
      ...
      // Abaikan perubahan lastSeen bila daftar siswa aktif tidak berubah.
...
(() => {
  ...
  function init(){
    ...
    // Dekorasi hanya perlu diulang setelah panel Algoritma benar-benar
    // dibangun ulang atau saat jumlah tim dipilih. Tidak perlu observer global.
    window.addEventListener("algorithm-teacher-monitor-rendered",()=>setTimeout(decorate,0));

    document.addEventListener("click",e=>{
      if(
        e.target.closest?.("[data-v22-team-count]") ||
        e.target.closest?.("[data-teacher-class]") ||
        e.target.closest?.("#teacherLoginBtn")
      ){
...
;/* ================= V47 SAFE TEXT SIZE CONTROL ================= */
(() => {
  "use strict";

  const STORAGE_KEY="kelasExcelTextScale";
  const DEFAULT_SCALE=100;
  const MIN_SCALE=85;
  const MAX_SCALE=130;
  const CONTROL_SELECTOR="button,input,select,textarea,option,[role='button']";
  const SCALE_UI_SELECTOR=".topbar,.student-score-badge,.exercise-score-card,.l2-side-list,.teacher-dashboard-top,.teacher-class-panel,.teacher-access-panel,.alg-tm-summary,.alg-tm-actions";
  const IGNORED_TAGS=new Set(["SCRIPT","STYLE","NOSCRIPT","TEMPLATE","SVG","PATH"]);

  const originalInline=new WeakMap();
  const baseSize=new WeakMap();
  ...
  function clampScale(raw){
    ...
    return Math.max(MIN_SCALE,Math.min(MAX_SCALE,Math.round(n/5)*5));
  }

  function readSavedScale(){
    try{return clampScale(localStorage.getItem(STORAGE_KEY));}
    catch(_){return DEFAULT_SCALE;}
  }

  function saveScale(next){
    try{localStorage.setItem(STORAGE_KEY,String(next));}catch(_){}
  }

  function hasDirectText(el){
    return Array.from(el.childNodes||[]).some(node=>
      node.nodeType===3 && String(node.nodeValue||node.textContent||"").trim()
    );
  }

  function isSafeTextTarget(el){
    if(!el||IGNORED_TAGS.has(el.tagName))return false;

    // Kontrol interaktif harus mempertahankan ukuran aslinya. Pada versi lama,
    // BUTTON/INPUT/SELECT/TEXTAREA ikut dibesarkan sehingga lebar/tinggi kontrol
    // berubah dan dapat mendorong grid atau membuat teks keluar dari tombol.
    if(el.matches?.(CONTROL_SELECTOR)||el.closest?.(CONTROL_SELECTOR))return false;

    // Panel pengatur ukuran teks tidak ikut diskalakan oleh dirinya sendiri.
    if(el.closest?.(".font-size-control,.brightness-control"))return false;
  ...
  function collectTextTargets(){
    ...
    if(!host)return [];
    return Array.from(host.querySelectorAll("*")).filter(isSafeTextTarget);
  }
  ...
  function scaleSensitivity(el,base){
    let sensitivity=1;

    // Label sangat kecil dan judul sangat besar diberi kurva lebih lembut agar
    // 130% tetap terbaca tanpa membuat chip/card melebar secara berlebihan.
    if(base<=10)sensitivity=.58;
    else if(base>=28)sensitivity=.76;

    // Teks di komponen UI padat tetap ikut membesar, tetapi dibatasi supaya
    // dimensi kontrol/layout tidak berubah drastis.
    if(el.closest?.(SCALE_UI_SELECTOR))sensitivity=Math.min(sensitivity,.62);
    return sensitivity;
  }

  function scaledFontSize(el,base){
    if(value===DEFAULT_SCALE)return base;
    const delta=(value-DEFAULT_SCALE)/100;
    const ratio=1+delta*scaleSensitivity(el,base);
  ...
  function paintTextScale(){
    ...
    currentTargets=currentTargets.filter(el=>el.isConnected);
    currentTargets.forEach(el=>{
      const base=baseSize.get(el);
      if(!Number.isFinite(base))return;
      if(value===DEFAULT_SCALE){
        restoreOriginal(el);
      }else{
        el.style.setProperty("font-size",`${scaledFontSize(el,base)}px`,"important");
      }
    });

    applying=false;
    window.ExerciseHeaderAccessV42?.sync?.();
  }
  ...
  function rebuildTextBases(){
    ...
    targets.forEach(el=>{
      ...
      if(Number.isFinite(measured))baseSize.set(el,measured);
    });

    for(const el of Array.from(managed)){
      if(el.isConnected&&!active.has(el)){
        restoreOriginal(el);
        managed.delete(el);
      }
    }
  ...
  function updateUi(){
    ...
    if(range){
      ...
      range.setAttribute("aria-valuetext",`${value} persen`);
    }

    document.querySelectorAll("[data-font-size]").forEach(btn=>{
      btn.classList.toggle("active",Number(btn.dataset.fontSize)===value);
    });
  }

  function applyTextScale(raw,{persist=true,rebuild=false}={}){
    value=clampScale(raw);
    document.documentElement.dataset.textScale=String(value);
    document.documentElement.style.setProperty("--kelas-text-scale",String(value/100));
    if(persist)saveScale(value);
    updateUi();
    if(rebuild||!currentTargets.length)rebuildTextBases();
    else schedulePaint();
  }

  function closeBrightnessPanel(){
    document.getElementById("brightnessPanel")?.classList.add("hidden");
    document.getElementById("brightnessToggle")?.setAttribute("aria-expanded","false");
  }
  ...
  function init(){
    value=readSavedScale();
    applyTextScale(value,{persist:false,rebuild:true});

    document.getElementById("fontSizeToggle")?.addEventListener("click",e=>{
      e.stopPropagation();
      togglePanel();
    });
    document.getElementById("fontSizePanel")?.addEventListener("click",e=>e.stopPropagation());
    document.getElementById("fontSizeRange")?.addEventListener("input",e=>applyTextScale(e.currentTarget.value));
    document.getElementById("fontSizeReset")?.addEventListener("click",()=>applyTextScale(DEFAULT_SCALE));
    document.querySelectorAll("[data-font-size]").forEach(btn=>{
      btn.addEventListener("click",()=>applyTextScale(btn.dataset.fontSize));
    });
    document.getElementById("brightnessToggle")?.addEventListener("click",closePanel);
    document.addEventListener("click",closePanel);
    document.addEventListener("keydown",e=>{if(e.key==="Escape")closePanel();});
    window.addEventListener("resize",scheduleRebuild,{passive:true});
    document.addEventListener("fullscreenchange",scheduleRebuild);
    ...
    if(host&&"MutationObserver" in window){
      observer=new MutationObserver(records=>{
        ...
        if(addedElement)scheduleRebuild();
      });
      observer.observe(host,{childList:true,subtree:true});
    }

    const api={
      version:"text-scale-v47-safe",
      get:()=>value,
      set:next=>applyTextScale(next),
      reset:()=>applyTextScale(DEFAULT_SCALE),
      refresh:scheduleRebuild
    };
    window.WebTextScaleV47=api;
    // Alias dipertahankan agar kode lama yang mungkin memanggil V43 tidak rusak.
    window.WebTextScaleV43=api;
  }
