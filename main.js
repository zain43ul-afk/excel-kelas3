...
(() => {
  ...
  const STATE={
    ...
    groupRefs:[],busy:false,timer:null
  };
  ...
  function attach(){
    ...
    detach();
    STATE.cls=cls;STATE.manager={};STATE.lobby={};
  ...
  function init(){
    ...
    const observer=new MutationObserver(()=>{
      clearTimeout(observer._lateJoinV41);
      observer._lateJoinV41=setTimeout(tick,80);
    });
    const teacherView=document.getElementById("teacherView");
    if(teacherView){
      observer.observe(teacherView,{subtree:true,childList:true,attributes:true,attributeFilter:["class"]});
    }
...
(() => {
  ...
  function init(){
    ...
    const teacherView=document.getElementById("teacherView");
    if(studentView)observer.observe(studentView,{subtree:true,childList:true,attributes:true,attributeFilter:["class"]});
    if(teacherView)observer.observe(teacherView,{subtree:true,childList:true,attributes:true,attributeFilter:["class"]});
...
(() => {
  ...
  function init(){
    ...
    const teacherView=document.getElementById("teacherView");
    if(teacherView){
      obs.observe(teacherView,{subtree:true,childList:true,attributes:true,attributeFilter:["class"]});
    }
...
(() => {
  ...
  const MIN_SCALE=85;
  const MAX_SCALE=130;
  const FORM_TAGS=new Set(["BUTTON","INPUT","SELECT","TEXTAREA","OPTION"]);
  ...
  function collectTextTargets(){
    ...
    return Array.from(host.querySelectorAll("*")).filter(el=>{
  ...
  function paintTextScale(){
    ...
    currentTargets.forEach(el=>{
      ...
      if(value===DEFAULT_SCALE){
        ...
        const scaled=Math.round(base*(value/100)*100)/100;
  ...
  function init(){
    ...
    window.WebTextScaleV43={
      version:"text-scale-v43",
      get:()=>value,
