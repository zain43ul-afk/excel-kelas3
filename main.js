...
;/* ================= V26 SMOOTH ALGORITHM PANEL ON CLASS SWITCH ================= */
(() => {
  "use strict";

  const VERSION="smooth-algorithm-class-panel-v26";
  ...
  function cleanup(immediate=false){
    ...
    const live=document.getElementById("algorithmTeacherMonitor");
    if(live){
      live.classList.remove(
        "alg-v26-live-hidden",
        "alg-v26-live-reveal",
        "alg-v26-live-switching"
      );
    ...
    if(state.snapshot){
      if(immediate){
        ...
        state.snapshot.classList.add("alg-v26-snapshot-hide");
        setTimeout(()=>state?.snapshot?.remove(),260);
      }
    }
  ...
  function finish(){
    ...
    // Jaga tinggi panel saat cross-fade agar halaman tidak meloncat.
    const currentHeight=Math.max(
      Number.parseFloat(live.style.minHeight)||0,
      live.getBoundingClientRect().height||0,
      state.originalHeight||0
    );
    if(currentHeight) live.style.minHeight=`${Math.round(currentHeight)}px`;

    live.classList.add("alg-v26-live-hidden");

    requestAnimationFrame(()=>{
      requestAnimationFrame(()=>{
        live.classList.remove("alg-v26-live-hidden","alg-v26-live-switching");
        live.classList.add("alg-v26-live-reveal");

        snapshot?.classList.add("alg-v26-snapshot-hide");

        setTimeout(()=>{
          snapshot?.remove();
          live.classList.remove("alg-v26-live-reveal");
          live.style.minHeight="";
          state=null;
        },280);
      });
  ...
  function scheduleFinish(){
    ...
    // Tunggu perubahan Firebase/DOM berhenti sebentar supaya beberapa render
    // internal tidak terlihat sebagai kedipan berulang.
    const elapsed=Date.now()-state.startedAt;
    const wait=elapsed<230 ? 230-elapsed+150 : 150;

    state.finishTimer=setTimeout(finish,wait);
  }
  ...
  function begin(tab){
    ...
    if(!next || next===current?.dataset?.teacherClass)return;
    ...
    snapshot.classList.add("alg-v26-panel-snapshot");
    ...
    live.style.minHeight=`${Math.round(rect.height)}px`;
    live.classList.add("alg-v26-live-switching");
    ...
    // Pantau panel asli. Setiap render internal menunda fade-out snapshot,
    // sehingga yang terlihat pengguna hanya satu perpindahan akhir.
    state.observer=new MutationObserver(scheduleFinish);
    state.observer.observe(live,{
      subtree:true,
      childList:true,
      characterData:true,
      attributes:true,
    ...
    // Jangan selesaikan terlalu cepat sebelum listener kelas baru mulai.
    state.minTimer=setTimeout(scheduleFinish,260);

    // Pengaman jika koneksi Firebase lambat/tidak memicu mutation lagi.
    state.maxTimer=setTimeout(finish,1250);
  }
  ...
  function updateSnapshotPosition(){
    ...
    Object.assign(state.snapshot.style,{
      left:`${Math.round(rect.left)}px`,
      top:`${Math.round(rect.top)}px`,
      width:`${Math.round(rect.width)}px`
    });
  ...
  function injectCss(){
    if(document.getElementById("algV26ClassPanelCss"))return;

    const style=document.createElement("style");
    style.id="algV26ClassPanelCss";
    style.textContent=`
      #algorithmTeacherMonitor.alg-v26-live-switching{
        transition:none !important;
      }

      #algorithmTeacherMonitor.alg-v26-live-hidden{
        opacity:0 !important;
        transform:translateY(-3px) scale(.997);
      }

      #algorithmTeacherMonitor.alg-v26-live-reveal{
        animation:algV26LiveReveal .26s cubic-bezier(.2,.8,.2,1) both;
      }

      .alg-v26-panel-snapshot{
        opacity:1;
        filter:none;
        transition:
          opacity .24s ease,
        ...
        box-shadow:0 10px 30px rgba(27,58,88,.05);
      }

      .alg-v26-panel-snapshot.alg-v26-snapshot-hide{
        opacity:0;
        transform:translateY(3px) scale(.997);
        filter:blur(.35px);
      }

      @keyframes algV26LiveReveal{
        from{
          opacity:.12;
  ...
  function init(){
    ...
    // Capture dijalankan sebelum handler kelas lama agar snapshot diambil
    // ketika panel kelas sebelumnya masih utuh.
    document.addEventListener("click",e=>{
      const tab=e.target.closest?.("[data-teacher-class]");
      if(!tab)return;
      begin(tab);
    ...
    window.addEventListener("scroll",updateSnapshotPosition,{passive:true});
    window.addEventListener("resize",updateSnapshotPosition,{passive:true});

    window.SmoothAlgorithmClassPanelV26={
      version:VERSION,
      finish,
      cleanup
    };
