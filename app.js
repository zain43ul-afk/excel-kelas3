(() => {
  ...
  function startNewMaterial(){
    if(!confirm("Mulai materi baru? Nilai, progres, status selesai, dan hasil latihan siswa yang sudah tersimpan TIDAK akan dihapus.")) return;

    // Mulai materi baru hanya mengubah status materi dan akses latihan.
    // Data siswa di kelasExcel/progress sama sekali tidak disentuh.
    ...
    if(realtime){
      db.ref("kelasExcel/settings").update({
        materialFinished:false,
        unlocked:settings.unlocked
      });
    }
