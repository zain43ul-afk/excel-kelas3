(() => {
  ...
  function renderL2(panel){
    ...
    const guideSlides=[
      ...
      {title:"2. Klik sel jawaban",text:"Klik sel Total atau Rata-rata yang ingin diisi. Contoh pertama adalah sel G2 untuk Total Buku Tulis.",mode:"target"},
      ...
      {title:"4. Tentukan blok angka",text:"Gunakan rentang B2:F2. Tanda titik dua berarti semua sel dari B2 sampai F2.",mode:"range"},
    ...
    const miniSheet=`<div class="l2-sim-sheet">
      ...
      <div class="l2-sim-grid">
        ...
        <div class="head">2</div><div class="label">Buku Tulis</div>${miniCells.map((v,i)=>`<div class="${gs.mode==="range"?'range':''}">${v}</div>`).join("")}<div class="${['target','type','range','done'].includes(gs.mode)?'focus':''}">${miniResult}</div>
      </div>
      <div class="l2-sim-status"><span>Latihan 2 • Tabel mendatar</span><b>Sel aktif: ${['target','type','range','done'].includes(gs.mode)?'G2':'–'}</b></div>
    </div>`;
