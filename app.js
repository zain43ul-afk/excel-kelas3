(() => {
  ...
  function scoreForExercise(index){
    ...
    const sec=(completedSec!==null && completedSec!==undefined && Number.isFinite(Number(completedSec)))
      ? Number(completedSec)
      : Number(currentUser.elapsedSeconds||0);
    return liveScoreFromElapsed(sec);
  }

  function updateLiveScoreDisplays(){
    if(!currentUser) return;
    const active=$("#exerciseScoreDisplay");
    if(active) active.textContent=String(scoreForExercise(currentExercise));
  ...
  function exerciseHeader(i,extra=""){
    ...
    return `<div class="exercise-window-header">
      ...
      <div class="exercise-score-card"><small>NILAI SAAT INI</small><strong id="exerciseScoreDisplay">${scoreForExercise(i)}</strong><span>turun 1 poin / menit</span></div>
  ...
  function renderL2(panel){
    ...
    panel.innerHTML=`
      <div class="l2-window-header">
        ...
        <div class="exercise-score-card"><small>NILAI SAAT INI</small><strong id="exerciseScoreDisplay">${scoreForExercise(1)}</strong><span>turun 1 poin / menit</span></div>
  ...
  function styledWindowHeader(index,title,themeClass=""){
    ...
    return `<div class="l2-window-header lx-window-header ${themeClass}">
      ...
      <div class="exercise-score-card"><small>NILAI SAAT INI</small><strong id="exerciseScoreDisplay">${scoreForExercise(index)}</strong><span>turun 1 poin / menit</span></div>
      <div class="l2-window-progress"><div><span>Progres keseluruhan</span><b>${percent}%</b></div><span><i style="width:${percent}%"></i></span><small>Waktu berjalan: <b id="exerciseWindowTimer">${formatTime(currentUser?.elapsedSeconds||0)}</b></small></div>
    </div>`;
  }
  ...
  function renderPracticeWindow(panel,cfg){
    ...
    panel.innerHTML=`
      <div class="l2-window-header l45-window-header ${practiceClass}">
        ...
        <div class="exercise-score-card"><small>NILAI SAAT INI</small><strong id="exerciseScoreDisplay">${scoreForExercise(cfg.index)}</strong><span>turun 1 poin / menit</span></div>
        <div class="l2-window-progress"><div><span>Progres keseluruhan</span><b>${percent}%</b></div><span><i style="width:${percent}%"></i></span><small>Waktu berjalan: <b id="exerciseWindowTimer">${formatTime(currentUser?.elapsedSeconds||0)}</b></small></div>
      </div>
  ...
  function renderSumifAverageif(panel){
    ...
    const guides=[
      {title:"1. Pilih sel hasil",text:"Klik sel Total F2 untuk SUMIF atau Rata-Rata G2 untuk AVERAGEIF. Sel jawaban aktif akan diberi garis biru.",mode:"target"},
      {title:"2. Ketik nama fungsi",text:"Ketik =SUMIF( atau =AVERAGEIF( secara manual. Sistem tidak menuliskan fungsi, koma, atau kurung untuk Anda.",mode:"function"},
      {title:"3. Blok rentang kriteria",text:"Klik-tahan dari B2 lalu seret sampai B5 pada simulasi. Rentang B2:B5 akan masuk ke rumus pada posisi kursor.",mode:"range1"},
      {title:"4. Klik sel kriteria",text:"Ketik koma, lalu klik E2 yang berisi Buku. Referensi E2 akan masuk ke rumus.",mode:"criteria"},
      {title:"5. Blok rentang jumlah",text:"Ketik koma, lalu blok C2:C5. Setelah referensi masuk, ketik kurung tutup ) sendiri.",mode:"range2"},
      {title:"6. Tekan Enter",text:"Jika rumus sudah lengkap, tekan Enter. Simulasi akan memeriksa rumus dan menampilkan hasil seperti lembar kerja asli.",mode:"enter"}
    ];
    const guide=guides[Math.max(0,Math.min(guides.length-1,st.guideSlide))];

    const multiGuides=[
      {title:"1. Pilih sel hasil",text:"Klik H2 untuk SUMIFS atau I2 untuk AVERAGEIFS. Sel tersebut menjadi tempat rumus dua kriteria.",mode:"target"},
      {title:"2. Ketik nama fungsi",text:"Ketik =SUMIFS( atau =AVERAGEIFS( sendiri. Jangan lupa tanda kurung buka.",mode:"function"},
      {title:"3. Blok rentang hasil",text:"Untuk SUMIFS/AVERAGEIFS, rentang angka berada di awal. Blok D2:D5 pada simulasi.",mode:"sumrange"},
      {title:"4. Blok kriteria pertama",text:"Ketik koma, blok B2:B5, ketik koma, lalu klik F2 untuk kriteria Jenis Item.",mode:"criteria1"},
      {title:"5. Blok kriteria kedua",text:"Ketik koma, blok C2:C5, ketik koma, lalu klik G2 untuk kriteria Cabang.",mode:"criteria2"},
      {title:"6. Tutup rumus",text:"Setelah semua referensi masuk, ketik tanda kurung tutup ) secara manual.",mode:"close"},
      {title:"7. Tekan Enter",text:"Tekan Enter untuk memeriksa rumus. Jika tepat, hasil muncul seperti pada lembar kerja asli.",mode:"enter"}
    ];
