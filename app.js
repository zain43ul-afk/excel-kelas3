(() => {
  ...
  function scoreForExercise(index){
    ...
    const isDone=!!currentUser.completedExercises?.[index];
    const completedSec=currentUser.exerciseCompletedSeconds?.[index];
    const liveSec=Math.max(0,Math.floor((((currentUser.completedAt||now())-(currentUser.startedAt||now()))/1000)));
    const sec=(isDone && completedSec!==null && completedSec!==undefined && Number.isFinite(Number(completedSec)))
      ? Number(completedSec)
      : liveSec;
  ...
  function renderPracticeWindow(panel,cfg){
    ...
    panel.innerHTML=`
      <div class="l2-window-header l45-window-header ${practiceClass}">
        ...
        <div class="exercise-score-card"><small>NILAI SAAT INI</small><strong data-live-active-score="1">${scoreForExercise(cfg.index)}</strong><span>turun 1 poin / menit</span></div>
        <div class="l2-window-progress"><div><span>Progres keseluruhan</span><b>${percent}%</b></div><span><i style="width:${percent}%"></i></span><small>Waktu berjalan: <b id="exerciseWindowTimer">${formatTime(currentUser?.elapsedSeconds||0)}</b></small></div>
      </div>
