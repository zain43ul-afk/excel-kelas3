(() => {
  ...
  function renderTeacher(){
    ...
    $("#lockControls").innerHTML=EXERCISES.map((e,i)=>{
      const open=teacherAccessPreview[i];
    ...
    $$("[data-preview-lock]").forEach(b=>b.addEventListener("click",()=>{
      ...
      teacherAccessPreview[i]=!teacherAccessPreview[i];
      renderTeacher();
      toast(`Tampilan guru: Latihan ${i+1} ${teacherAccessPreview[i]?"dibuka":"dikunci"}. Belum dikirim ke siswa.`);
    }));
    ...
    $("#teacherTableBody").innerHTML=classRecords.map(r=>{
      ...
      return `<tr>
        ...
        ${(r.completedExercises||Array(8).fill(false)).map(x=>`<td class="exercise-check">${x?"✓":"—"}</td>`).join("")}
        <td>
  ...
  function startNewMaterial(){
    ...
    if(realtime){
      ...
      db.ref("kelasExcel/settings/materialFinished").set(false);
    }
    renderTeacher();
