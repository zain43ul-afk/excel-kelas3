(() => {
  ...
  function renderL2(panel){
    ...
    const allExtraCorrect=allExtra.every((q,i)=>Number(st.extra[i<5?`h${i}`:`v${i-5}`])===Number(q.answer));
    ...
    const guideSlides=[
      ...
      {title:"4. Tentukan blok angka",text:"Klik-tahan sel B2 lalu seret sampai F2. Blok yang dipilih akan membentuk rentang B2:F2. Tanda titik dua berarti semua sel dari B2 sampai F2.",mode:"range"},
      {title:"5. Tekan Enter",text:"Setelah rumus lengkap, tekan Enter. Jika benar, sel menampilkan hasil dan soal tambahan akan terbuka setelah semua sel selesai.",mode:"done"}
    ];
    const gs=guideSlides[Math.max(0,Math.min(4,st.guideSlide))];
    const miniCells=[12,35,35,24,14];
    const miniFormula=gs.mode==="type"?"=SUM(":gs.mode==="range"?"=SUM(B2:F2)":gs.mode==="done"?"=SUM(B2:F2)":"Klik sel jawaban untuk mulai";
    const miniResult=gs.mode==="done"?"120":"";
    const miniSheet=`<div class="l2-sim-sheet">
      <div class="l2-sim-fx"><span>fx</span><b>${miniFormula}</b></div>
      ...
      <div class="l2-layout">
        <main class="l2-main">
          ...
          <section class="l2-section-card">
            ...
            <div class="l2-extra-box"><div class="l2-extra-title"><div><h4>5 soal tambahan</h4><p>Isi semua Total dan Rata-rata, lalu tekan Enter pada setiap rumus untuk membuka soal tambahan.</p></div><span>${hExtraCount}/5 terisi</span></div><div class="l2-extra-grid">${extraCards(hExtra,'h',hCorrect)}</div></div>
          </section>
          ...
          <section class="l2-section-card">
            ...
            <div class="l2-extra-box"><div class="l2-extra-title"><div><h4>5 soal tambahan</h4><p>Isi semua Total dan Rata-rata, lalu tekan Enter pada setiap rumus untuk membuka soal tambahan.</p></div><span>${vExtraCount}/5 terisi</span></div><div class="l2-extra-grid">${extraCards(vExtra,'v',vCorrect)}</div></div>
          </section>
        ...
        <aside class="l2-sidebar">
          ...
          <section class="l2-quick-tip"><h4>Petunjuk cepat</h4><p>Ketik nama fungsi dan tanda kurung buka lebih dahulu. Setelah itu blok rentang angka pada tabel yang sama.</p><div>=SUM( + blok B2:F2 → =SUM(B2:F2)</div></section>
        </aside>
      </div>`;
    ...
    const bindL2RangeSelection=()=>{
      ...
      const applyRangeToFormula=(table,range)=>{
        if(!activeInput||!activeInput.isConnected||activeInput.closest("table")!==table){showBlock(range);return;}
        const expected=activeInput.dataset.expected||"";
        const current=activeInput.value.trim();
        const curFn=current.match(/^=(SUM|AVERAGE)\s*\(/i);
        const expFn=expected.match(/^=(SUM|AVERAGE)\s*\(/i);
        const fn=(curFn?.[1]||expFn?.[1]||"SUM").toUpperCase();
    ...
    const bindL2Simulation=()=>{
      ...
      const fx=panel.querySelector(".l2-sim-fx b");
      ...
      target.addEventListener("click",()=>{
        targetActive=true;target.classList.add("l2-sim-target-active");fx.textContent="=SUM(";status.textContent="Sel aktif: G2 • sekarang blok B2:F2";enter.disabled=true;
      });
      grid.addEventListener("pointerdown",e=>{
        ...
        range=selectedRange(drag.start,drag.end);fx.textContent=`=SUM(${range})`;status.textContent=`Blok: ${range}`;
      });
      grid.addEventListener("pointermove",e=>{
        ...
        drag.end=cell.dataset.l2SimSource;range=selectedRange(drag.start,drag.end);fx.textContent=`=SUM(${range})`;status.textContent=`Blok: ${range}`;
      });
      const done=e=>{
        if(!drag||drag.id!==e.pointerId) return;
        range=selectedRange(drag.start,drag.end);drag=null;fx.textContent=`=SUM(${range})`;status.textContent=`Rumus: =SUM(${range})`;enter.disabled=false;
      ...
      enter.addEventListener("click",()=>{
        ...
        const total=selected.reduce((sum,x)=>sum+Number(x.textContent||0),0);
        if(!selected.length) return;
        target.textContent=String(total);target.classList.add("l2-sim-done");status.textContent=`G2 = ${total}`;enter.disabled=true;
      });
    };
    ...
    $$("[data-l2-extra]").forEach(inp=>inp.addEventListener("input",()=>{
    ...
    if(check) check.addEventListener("click",async()=>{
      ...
      allExtra.forEach((q,i)=>{
        ...
        const ok=Number(st.extra[key])===Number(q.answer);
