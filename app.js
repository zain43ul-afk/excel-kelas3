(() => {
  ...
  function renderSumifAverageif(panel){
    const st=getExState(3,{answers:{},guideSlide:0,multiGuideSlide:0,readyToFinish:false,active:null});
    st.answers ||= {};
    st.guideSlide=Number.isInteger(st.guideSlide)?st.guideSlide:0;
    ...
    const oneCriteria=[
      ...
      ...x,
      sumFormula:`=SUMIF($B$2:$B$21,E${i+2},$C$2:$C$21)`,
      avgFormula:`=AVERAGEIF($B$2:$B$21,E${i+2},$C$2:$C$21)`
    }));
    ...
    const multiCriteria=[
      ...
      ...x,
      sumFormula:`=SUMIFS($D$2:$D$21,$B$2:$B$21,F${i+2},$C$2:$C$21,G${i+2})`,
      avgFormula:`=AVERAGEIFS($D$2:$D$21,$B$2:$B$21,F${i+2},$C$2:$C$21,G${i+2})`
    }));
    ...
    const formulaCell=(task)=>{
      ...
      if(a.correct){
        return `<button type="button" class="l4-formula-result" data-l4-active="${task.key}">
          ...
          <small>${escapeHtml(task.formula)}</small>
        </button>`;
      }
    ...
    const oneBody=oneRows.map((r,i)=>{
      ...
      return `<tr>
        ...
        <td>${c?escapeHtml(c.item):""}</td>
        <td class="l4-answer-cell">${sumTask?formulaCell(sumTask):""}</td>
        <td class="l4-answer-cell">${avgTask?formulaCell(avgTask):""}</td>
      </tr>`;
    }).join("");
    ...
    const guides=[
      {title:"1. Kenali lembar A",text:"Pada lembar A, kolom B berisi Jenis Item dan kolom C berisi Jumlah. Daftar item yang harus dihitung berada di kolom E.",formula:'B = Jenis Item • C = Jumlah • E = kriteria'},
      {title:"2. Pilih sel hasil",text:"Klik sel Total di kolom F atau Rata-Rata di kolom G pada baris item yang ingin dihitung. Contoh pertama memakai item Buku pada E2.",formula:'E2 = Buku → hasil ditulis di F2 dan G2'},
      {title:"3. Tulis SUMIF",text:"SUMIF dipakai untuk menjumlahkan satu kriteria. Urutannya adalah rentang kriteria, kriteria, lalu rentang angka yang dijumlahkan.",formula:'=SUMIF($B$2:$B$21,E2,$C$2:$C$21)'},
      {title:"4. Tulis AVERAGEIF",text:"AVERAGEIF dipakai untuk menghitung rata-rata dengan satu kriteria. Polanya hampir sama dengan SUMIF.",formula:'=AVERAGEIF($B$2:$B$21,E2,$C$2:$C$21)'},
      {title:"5. Salin pola ke baris berikutnya",text:"Untuk Pensil, Penghapus, Pulpen, dan Penggaris, rentang data tetap sama. Yang berubah hanya sel kriterianya: E3, E4, E5, dan E6.",formula:'E2 → E3 → E4 → E5 → E6'},
      {title:"6. Tekan Enter dan cek hasil",text:"Setelah rumus lengkap, tekan Enter. Jika benar, sel akan menampilkan angka hasil serta rumus yang digunakan.",formula:'Enter → rumus diperiksa → hasil tampil'}
    ];
    const guide=guides[Math.max(0,Math.min(guides.length-1,st.guideSlide))];

    const guideMini=`<div class="l4-guide-mini l4-guide-mini-one">
      <div class="l4-guide-mini-grid one">
        <span>Jenis Item</span><span>Jumlah</span><span>Hasil</span>
        <b>Buku</b><b>12</b><b>F2 / G2</b>
        <b>Buku</b><b>14</b><b></b>
      </div>
      <code>${escapeHtml(guide.formula)}</code>
    </div>`;

    const multiGuides=[
      {title:"1. Kenali lembar B",text:"Pada lembar B, data utama menggunakan Jenis Item di kolom B, Cabang di kolom C, dan Jumlah di kolom D. Kriteria hasil berada di kolom F dan G.",formula:'B = Item • C = Cabang • D = Jumlah'},
      {title:"2. Pahami dua kriteria",text:"Setiap hasil harus memenuhi dua syarat sekaligus: Jenis Item dan Cabang. Contoh pertama adalah Beras pada Cabang A.",formula:'F2 = Beras • G2 = A'},
      {title:"3. Tulis SUMIFS",text:"SUMIFS meletakkan rentang yang dijumlahkan di awal, lalu pasangan rentang kriteria dan kriterianya.",formula:'=SUMIFS($D$2:$D$21,$B$2:$B$21,F2,$C$2:$C$21,G2)'},
      {title:"4. Periksa urutan SUMIFS",text:"Jangan tertukar dengan SUMIF. Untuk SUMIFS: sum_range lebih dulu, kemudian criteria_range1, criteria1, criteria_range2, criteria2.",formula:'SUMIFS(sum_range,range1,kriteria1,range2,kriteria2)'},
      {title:"5. Tulis AVERAGEIFS",text:"AVERAGEIFS memakai pola yang sama, tetapi rentang pertama adalah angka yang akan dirata-ratakan.",formula:'=AVERAGEIFS($D$2:$D$21,$B$2:$B$21,F2,$C$2:$C$21,G2)'},
      {title:"6. Salin pola untuk kriteria lain",text:"Untuk Susu-C, Kopi-B, Gula-B, dan Teh-A, rentang data tetap sama. Hanya referensi kriteria F dan G yang berpindah ke baris berikutnya.",formula:'F2/G2 → F3/G3 → F4/G4 → F5/G5 → F6/G6'},
      {title:"7. Tekan Enter dan cek hasil",text:"Tekan Enter pada setiap sel Total dan Rata-Rata. Jika rumus tepat, hasil akan tampil dan indikator benar bertambah.",formula:'Enter → periksa 10 rumus bagian B'}
    ];
    const multiGuide=multiGuides[Math.max(0,Math.min(multiGuides.length-1,st.multiGuideSlide))];
    const multiGuideMini=`<div class="l4-guide-mini l4-guide-mini-multi">
      <div class="l4-guide-mini-grid multi">
        <span>Jenis Item</span><span>Cabang</span><span>Jumlah</span>
        <b>Beras</b><b>A</b><b>25</b>
        <b>Beras</b><b>A</b><b>23</b>
      </div>
      <code>${escapeHtml(multiGuide.formula)}</code>
    </div>`;
        ...
        <main class="l2-main">
          <section class="l2-learning-row">
            <div class="l2-material-card">
              ...
              <div>
                <code>=SUMIF(range,kriteria,sum_range)</code>
                <code>=AVERAGEIF(range,kriteria,average_range)</code>
              </div>
            </div>
            <div class="l2-guide-card">
              <div class="l2-guide-head"><h4>☷ &nbsp; Petunjuk Pengerjaan</h4><span>${st.guideSlide+1}/${guides.length}</span></div>
              <div class="l2-guide-slide"><h3>${escapeHtml(guide.title)}</h3><p>${escapeHtml(guide.text)}</p>${guideMini}</div>
              <div class="l2-guide-nav">
          ...
          <section class="l2-instruction-band">
            Isi sel <b>Total</b> dan <b>Rata-Rata</b> pada dua lembar kerja. Tekan <b>Enter</b> setelah setiap rumus. Total ada <b>20 jawaban</b>.
          </section>

          <section class="l2-section-card">
            <div class="l2-section-head">
              <div><h3>A. SUMIF & AVERAGEIF — satu kriteria</h3><p>Lembar kerja mengikuti contoh: data utama di kiri dan daftar Jenis Item beserta hasil di kanan.</p></div>
              <span>${tasks.slice(0,10).filter(t=>st.answers[t.key]?.correct).length}/10 benar</span>
            </div>
            <div class="l4-sheet-wrap">
              <table class="l4-sheet l4-sheet-one">
                <colgroup>
                  <col class="rowhead"><col class="no"><col class="item"><col class="qty"><col class="gap"><col class="criteria"><col class="result"><col class="result">
                </colgroup>
                <thead>
                  <tr class="letters"><th></th><th>A</th><th>B</th><th>C</th><th>D</th><th>E</th><th>F</th><th>G</th></tr>
                  <tr><th>1</th><th>No</th><th>Jenis Item</th><th>Jumlah</th><th></th><th>Jenis Item</th><th>Total</th><th>Rata-Rata</th></tr>
                </thead>
                <tbody>${oneBody}</tbody>
              </table>
            </div>
          ...
          <section class="l4-secondary-learning">
            <div class="l4-secondary-material">
              <span>Materi lanjutan</span>
              <h3>SUMIFS & AVERAGEIFS</h3>
              <p>Gunakan fungsi berakhiran <b>S</b> ketika perhitungan memakai <b>dua atau lebih kriteria</b>. Pada lembar B, syaratnya adalah Jenis Item dan Cabang.</p>
              <code>=SUMIFS(sum_range,range1,kriteria1,range2,kriteria2)</code>
              <code>=AVERAGEIFS(avg_range,range1,kriteria1,range2,kriteria2)</code>
            </div>
            <div class="l2-guide-card l4-secondary-guide">
              ...
              <div class="l2-guide-slide"><h3>${escapeHtml(multiGuide.title)}</h3><p>${escapeHtml(multiGuide.text)}</p>${multiGuideMini}</div>
              <div class="l2-guide-nav">
                <button type="button" data-l4-multi-prev ${st.multiGuideSlide===0?'disabled':''}>‹ &nbsp; Sebelumnya</button>
                <div>${multiGuides.map((_,i)=>`<i class="${i===st.multiGuideSlide?'active':''}"></i>`).join("")}</div>
                <button type="button" data-l4-multi-next ${st.multiGuideSlide===multiGuides.length-1?'disabled':''}>Berikutnya &nbsp; ›</button>
              </div>
            </div>
          </section>
          ...
          <section class="l2-section-card">
            <div class="l2-section-head">
              <div><h3>B. SUMIFS & AVERAGEIFS — dua kriteria</h3><p>Gunakan dua syarat sekaligus: <b>Jenis Item</b> dan <b>Cabang</b>.</p></div>
              <span>${tasks.slice(10).filter(t=>st.answers[t.key]?.correct).length}/10 benar</span>
            </div>
            <div class="l4-sheet-wrap">
              <table class="l4-sheet l4-sheet-multi">
                <colgroup>
                  <col class="rowhead"><col class="no"><col class="item"><col class="branch"><col class="qty"><col class="gap"><col class="criteria"><col class="branch"><col class="result"><col class="result">
                </colgroup>
                <thead>
                  <tr class="letters"><th></th><th>A</th><th>B</th><th>C</th><th>D</th><th>E</th><th>F</th><th>G</th><th>H</th><th>I</th></tr>
                  <tr><th>1</th><th>No</th><th>Jenis Item</th><th>Cabang</th><th>Jumlah</th><th></th><th>Jenis Item</th><th>Cabang</th><th>Total</th><th>Rata-Rata</th></tr>
                </thead>
          ...
          <button id="l4CheckAll" class="l2-check-all ${st.readyToFinish&&allCorrect?'ready':''}" ${!allFilled&&!st.readyToFinish?'disabled':''}>
            ${done?'✓ Latihan selesai':st.readyToFinish&&allCorrect?'✓ Selesai':`✓ Periksa seluruh 20 jawaban`}
          </button>
        </main>

        <aside class="l2-sidebar">
          ${styledParticipant(3)}
          <section class="l2-side-card">
            <h4>Deteksi rumus</h4>
            <p>Klik salah satu sel Total atau Rata-Rata.</p>
            <div id="l4Detection">${st.active?`<b>${escapeHtml(st.active.addr)}</b><span>${escapeHtml(st.active.value||'Belum diisi')}</span>`:'Belum ada jawaban dipilih.'}</div>
          </section>
          <section class="l2-quick-tip">
            <h4>Petunjuk cepat</h4>
            <p><b>SUMIF/AVERAGEIF</b> = satu syarat. <b>SUMIFS/AVERAGEIFS</b> = beberapa syarat. Perhatikan bahwa SUMIFS dan AVERAGEIFS meletakkan rentang hasil di awal.</p>
            <div>=SUMIFS($D$2:$D$21,$B$2:$B$21,F2,$C$2:$C$21,G2)</div>
          </section>
        </aside>
      </div>`;
    ...
    const setDetection=(key)=>{
      ...
      if(!t) return;
      const a=st.answers[key]||{};
      st.active={addr:t.addr,label:t.label,value:a.value||""};
    ...
    panel.querySelectorAll("[data-l4-key]").forEach(inp=>{
      inp.addEventListener("focus",()=>setDetection(inp.dataset.l4Key));
      inp.addEventListener("input",()=>{
      ...
      inp.addEventListener("keydown",async e=>{
        ...
        const key=inp.dataset.l4Key;
        const t=tasks.find(x=>x.key===key);
        const correct=t && normalizePracticeFormula(inp.value)===normalizePracticeFormula(t.formula);
        st.answers[key]={value:inp.value,correct:!!correct};
        st.readyToFinish=false;
        const scroll=keepScroll();
        try{await persistStudent();}catch(_){}
        rerender(scroll);
        toast(correct?`Benar. Hasil ${t.addr} = ${formatResult(t.result)}`:"Rumus belum tepat. Periksa kembali fungsi, rentang, dan kriterianya.");
      });
    });
    ...
    if(check) check.addEventListener("click",async()=>{
      if(done) return;
      if(st.readyToFinish && tasks.every(t=>st.answers[t.key]?.correct===true)){
        await completeExercise(3);
        return;
      }
      if(!tasks.every(t=>String(st.answers[t.key]?.value||"").trim())){
        toast("Isi seluruh 20 rumus terlebih dahulu.");
        return;
      }
      tasks.forEach(t=>{
        const value=st.answers[t.key]?.value||"";
      ...
      try{await persistStudent();}catch(_){}
      rerender(scroll);
      toast(ok?`Semua 20 rumus benar. Klik Selesai untuk mengirim hasil.`:`${tasks.filter(t=>st.answers[t.key]?.correct).length} dari 20 rumus benar. Perbaiki rumus yang masih salah.`);
    });
