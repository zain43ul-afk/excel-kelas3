(() => {
  ...
  function scoreForExercise(index){
    ...
    const completedSec=currentUser.exerciseCompletedSeconds?.[index];
    const sec=(completedSec!==null && completedSec!==undefined && Number.isFinite(Number(completedSec)))
      ? Number(completedSec)
      : Math.max(0,Math.floor((((currentUser.completedAt||now())-(currentUser.startedAt||now()))/1000)));
    return liveScoreFromElapsed(sec);
  }

  function updateLiveScoreDisplays(){
    if(!currentUser) return;
    $$("[data-live-active-score]").forEach(active=>active.textContent=String(scoreForExercise(currentExercise)));
    $$("[data-live-score-ex]").forEach(el=>{
      const i=Number(el.dataset.liveScoreEx);
      if(Number.isInteger(i)) el.textContent=String(scoreForExercise(i));
    });
  }
    ...
    $("#studentExerciseNav").innerHTML=EXERCISES.map((e,i)=>{
      ...
      return `<button type="button" data-ex="${i}" class="student-window-card theme-${themes[i]} ${open?"open":"locked"} ${done?"done":""}">
        <span class="student-window-top">
          ...
          <span class="student-score-badge"><small>NILAI</small><strong data-live-score-ex="${i}">${scoreForExercise(i)}</strong></span>
  ...
  function exerciseHeader(i,extra=""){
    ...
    return `<div class="exercise-window-header">
      ...
      <div class="exercise-score-card"><small>NILAI SAAT INI</small><strong data-live-active-score="1">${scoreForExercise(i)}</strong><span>turun 1 poin / menit</span></div>
  ...
  function renderL2(panel){
    ...
    panel.innerHTML=`
      <div class="l2-window-header">
        ...
        <div class="exercise-score-card"><small>NILAI SAAT INI</small><strong data-live-active-score="1">${scoreForExercise(1)}</strong><span>turun 1 poin / menit</span></div>
  ...
  function styledWindowHeader(index,title,themeClass=""){
    ...
    return `<div class="l2-window-header lx-window-header ${themeClass}">
      ...
      <div class="exercise-score-card"><small>NILAI SAAT INI</small><strong data-live-active-score="1">${scoreForExercise(index)}</strong><span>turun 1 poin / menit</span></div>
      <div class="l2-window-progress"><div><span>Progres keseluruhan</span><b>${percent}%</b></div><span><i style="width:${percent}%"></i></span><small>Waktu berjalan: <b id="exerciseWindowTimer">${formatTime(currentUser?.elapsedSeconds||0)}</b></small></div>
    </div>`;
  }

  function styledParticipant(index){
    return `<section class="l2-participant"><small>PESERTA</small><h4>${escapeHtml(currentUser.name)}</h4><p>Kelas ${escapeHtml(currentUser.className)}</p><button type="button" data-exit-student ${settings.materialFinished?'':'disabled'}>↪ &nbsp; Keluar</button><em>${settings.materialFinished?'Materi selesai':'Keluar aktif setelah guru menekan Materi selesai.'}</em></section>
  ...
  function renderL3(panel){
    ...
    const guides=[
      {title:"1. Pilih sel hasil",text:"Mulai dari sel hasil di tabel kanan. Jika ingin SUMIF, klik sel Total F2. Jika ingin AVERAGEIF, klik sel Rata-Rata G2.",mode:"target"},
      {title:"2. Tulis fungsi pada kotak rumus",text:"Ketik fungsi secara manual di kotak rumus (fx) di bagian paling atas simulasi. Sistem tidak menuliskan fungsi untuk Anda.",mode:"function"},
      {title:"3. Blok rentang kriteria",text:"Setelah tanda kurung buka, blok kolom Jenis Item dari B2 sampai B5. Rentang ini menjadi tempat pencarian kriteria.",mode:"range1"},
      {title:"4. Klik sel kriterianya",text:"Setelah mengetik koma, klik sel E2 yang berisi Buku. Inilah sel kriteria yang akan dicocokkan dengan rentang B2:B5.",mode:"criteria"},
      {title:"5. Blok rentang jumlah / nilai",text:"Setelah mengetik koma lagi, blok C2:C5 sebagai rentang angka yang akan dijumlahkan atau dirata-ratakan. Lalu ketik kurung tutup ) sendiri.",mode:"range2"},
      {title:"6. Tekan Enter dan cek hasil",text:"Jika rumus lengkap, tekan Enter. Jika benar, hasil akan muncul pada sel F2 atau G2 seperti di lembar kerja asli.",mode:"enter"}
    ];
    const guide=guides[Math.max(0,Math.min(guides.length-1,st.guideSlide))];

    const multiGuides=[
      {title:"1. Pilih sel hasil",text:"Untuk SUMIFS klik H2, untuk AVERAGEIFS klik I2. Kedua sel itu adalah tempat hasil rumus dua kriteria.",mode:"target"},
      {title:"2. Tulis fungsi pada kotak rumus",text:"Tulis =SUMIFS( atau =AVERAGEIFS( di kotak rumus (fx) bagian atas. Fungsi harus diketik sendiri.",mode:"function"},
      {title:"3. Blok rentang jumlah di awal",text:"Pada SUMIFS/AVERAGEIFS, rentang angka diletakkan paling awal. Blok D2:D5 terlebih dahulu.",mode:"sumrange"},
      {title:"4. Blok rentang kriteria 1 lalu klik kriterianya",text:"Ketik koma, blok B2:B5, ketik koma, lalu klik F2 sebagai kriteria pertama (Jenis Item).",mode:"criteria1"},
      {title:"5. Blok rentang kriteria 2 lalu klik kriterianya",text:"Ketik koma, blok C2:C5, ketik koma, lalu klik G2 sebagai kriteria kedua (Cabang).",mode:"criteria2"},
      {title:"6. Tutup rumus",text:"Setelah semua referensi masuk, ketik kurung tutup ) sendiri di kotak rumus.",mode:"close"},
      {title:"7. Tekan Enter dan cek hasil",text:"Tekan Enter untuk memeriksa rumus. Jika benar, hasil tampil di H2 atau I2 seperti pada lembar kerja.",mode:"enter"}
    ];
    const multiGuide=multiGuides[Math.max(0,Math.min(multiGuides.length-1,st.multiGuideSlide))];

    const renderGuideCallout=meta=>`<div class="l4-guide-callout">
      <div><span>Tulis di kotak fx</span><b>${escapeHtml(meta.write||'Belum menulis rumus')}</b></div>
      <div><span>Sel yang diklik</span><b>${escapeHtml(meta.click||'Belum ada')}</b></div>
      <div><span>Rentang yang diblok</span><b>${escapeHtml(meta.block||'Belum ada')}</b></div>
      <div><span>Keterangan</span><b>${escapeHtml(meta.note||'Ikuti contoh sesuai gambar.')}</b></div>
    </div>`;

    const oneMeta=(()=>{
      switch(guide.mode){
        case 'target': return {target:'F2',write:'Belum ada. Mulai dengan klik F2 atau G2.',click:'Klik F2 (Total) atau G2 (Rata-Rata)',block:'Belum ada',note:'Sel hasil aktif akan berwarna biru.'};
        case 'function': return {target:'F2',write:'=SUMIF(  atau  =AVERAGEIF(',click:'F2 sudah aktif, lalu mulai mengetik',block:'Belum ada',note:'Penulisan fungsi dilakukan di kotak rumus (fx) di atas tabel kecil.',fx:'=SUMIF('};
        case 'range1': return {target:'F2',write:'=SUMIF(',click:'F2 tetap aktif',block:'Blok B2:B5',note:'Rentang kriteria berasal dari kolom Jenis Item.',fx:'=SUMIF(',selected:['B2','B3','B4','B5']};
        case 'criteria': return {target:'F2',write:'=SUMIF(B2:B5,',click:'Klik E2',block:'Sudah ada B2:B5',note:'E2 adalah sel kriteria yang berisi Buku.',fx:'=SUMIF(B2:B5,',selected:['E2']};
        case 'range2': return {target:'F2',write:'=SUMIF(B2:B5,E2,',click:'E2 sudah dimasukkan',block:'Blok C2:C5',note:'Setelah rentang C2:C5 masuk, ketik ) sendiri.',fx:'=SUMIF(B2:B5,E2,',selected:['C2','C3','C4','C5']};
        default: return {target:'F2',write:'=SUMIF(B2:B5,E2,C2:C5)',click:'Tidak ada klik baru',block:'Semua referensi sudah lengkap',note:'Tekan Enter untuk menampilkan hasil 43 pada F2.',fx:'=SUMIF(B2:B5,E2,C2:C5)',done:{addr:'F2',value:'43'}};
      }
    })();

    const multiMeta=(()=>{
      switch(multiGuide.mode){
        case 'target': return {target:'H2',write:'Belum ada. Klik H2 atau I2 lebih dulu.',click:'Klik H2 (Total) atau I2 (Rata-Rata)',block:'Belum ada',note:'Sel hasil aktif akan diberi garis biru.'};
        case 'function': return {target:'H2',write:'=SUMIFS(  atau  =AVERAGEIFS(',click:'H2 sudah aktif, lalu mulai mengetik',block:'Belum ada',note:'Tulisan dimasukkan pada kotak rumus (fx).',fx:'=SUMIFS('};
        case 'sumrange': return {target:'H2',write:'=SUMIFS(',click:'H2 tetap aktif',block:'Blok D2:D5',note:'Pada SUMIFS/AVERAGEIFS, rentang jumlah berada di paling awal.',fx:'=SUMIFS(',selected:['D2','D3','D4','D5']};
        case 'criteria1': return {target:'H2',write:'=SUMIFS(D2:D5,',click:'Klik F2 setelah blok B2:B5',block:'Blok B2:B5',note:'B2:B5 adalah rentang kriteria 1, F2 adalah nilai kriterianya.',fx:'=SUMIFS(D2:D5,B2:B5,',selected:['B2','B3','B4','B5','F2']};
        case 'criteria2': return {target:'H2',write:'=SUMIFS(D2:D5,B2:B5,F2,',click:'Klik G2 setelah blok C2:C5',block:'Blok C2:C5',note:'C2:C5 adalah rentang kriteria 2, G2 adalah nilai kriterianya.',fx:'=SUMIFS(D2:D5,B2:B5,F2,C2:C5,',selected:['C2','C3','C4','C5','G2']};
        case 'close': return {target:'H2',write:'=SUMIFS(D2:D5,B2:B5,F2,C2:C5,G2)',click:'Tidak ada klik baru',block:'Semua referensi sudah lengkap',note:'Sekarang tambahkan kurung tutup ) di akhir rumus.',fx:'=SUMIFS(D2:D5,B2:B5,F2,C2:C5,G2)'};
        default: return {target:'H2',write:'=SUMIFS(D2:D5,B2:B5,F2,C2:C5,G2)',click:'Tidak ada klik baru',block:'Rumus sudah lengkap',note:'Tekan Enter untuk menampilkan hasil 48 pada H2.',fx:'=SUMIFS(D2:D5,B2:B5,F2,C2:C5,G2)',done:{addr:'H2',value:'48'}};
      }
    })();

    const simRef=(addr,text,selected=[])=>`<div data-l4-sim-ref="${addr}" class="${selected.includes(addr)?'selected':''}">${text}</div>`;
    const simTarget=(addr,text,active,done)=>`<button type="button" data-l4-sim-target="${addr}" class="${active===addr?'active':''} ${done&&done.addr===addr?'done':''}">${done&&done.addr===addr?done.value:text}</button>`;

    const oneSimValue=escapeHtml(oneMeta.fx||"");
    const multiSimValue=escapeHtml(multiMeta.fx||"");
    const oneSim=`<div class="l4-real-sim" data-l4-sim="one">
      ${renderGuideCallout(oneMeta)}
      <div class="l4-sim-formula-row"><span>fx</span><input id="l4SimOneFx" value="${oneSimValue}" placeholder="Tulis rumus di sini"><button type="button" id="l4SimOneEnter">Enter</button></div>
      <div class="l4-sim-sheet one">
        ...
        <div class="rowhead">2</div>${simRef('B2','Buku',oneMeta.selected||[])}${simRef('C2','12',oneMeta.selected||[])}<div class="gap"></div>${simRef('E2','Buku',oneMeta.selected||[])}${simTarget('F2','F2',oneMeta.target,oneMeta.done)}${simTarget('G2','G2',oneMeta.target,oneMeta.done)}
        <div class="rowhead">3</div>${simRef('B3','Pensil',oneMeta.selected||[])}${simRef('C3','18',oneMeta.selected||[])}<div class="gap"></div><div></div><div></div><div></div>
        <div class="rowhead">4</div>${simRef('B4','Buku',oneMeta.selected||[])}${simRef('C4','14',oneMeta.selected||[])}<div class="gap"></div><div></div><div></div><div></div>
        <div class="rowhead">5</div>${simRef('B5','Buku',oneMeta.selected||[])}${simRef('C5','17',oneMeta.selected||[])}<div class="gap"></div><div></div><div></div><div></div>
      </div>
      <div class="l4-sim-status" id="l4SimOneStatus">${escapeHtml(oneMeta.note)}</div>
    </div>`;

    const multiSim=`<div class="l4-real-sim" data-l4-sim="multi">
      ${renderGuideCallout(multiMeta)}
      <div class="l4-sim-formula-row"><span>fx</span><input id="l4SimMultiFx" value="${multiSimValue}" placeholder="Tulis rumus di sini"><button type="button" id="l4SimMultiEnter">Enter</button></div>
      <div class="l4-sim-sheet multi">
        ...
        <div class="rowhead">2</div>${simRef('B2','Beras',multiMeta.selected||[])}${simRef('C2','A',multiMeta.selected||[])}${simRef('D2','25',multiMeta.selected||[])}<div class="gap"></div>${simRef('F2','Beras',multiMeta.selected||[])}${simRef('G2','A',multiMeta.selected||[])}${simTarget('H2','H2',multiMeta.target,multiMeta.done)}${simTarget('I2','I2',multiMeta.target,multiMeta.done)}
        <div class="rowhead">3</div>${simRef('B3','Beras',multiMeta.selected||[])}${simRef('C3','B',multiMeta.selected||[])}${simRef('D3','30',multiMeta.selected||[])}<div class="gap"></div><div></div><div></div><div></div><div></div>
        <div class="rowhead">4</div>${simRef('B4','Beras',multiMeta.selected||[])}${simRef('C4','A',multiMeta.selected||[])}${simRef('D4','23',multiMeta.selected||[])}<div class="gap"></div><div></div><div></div><div></div><div></div>
        <div class="rowhead">5</div>${simRef('B5','Gula',multiMeta.selected||[])}${simRef('C5','B',multiMeta.selected||[])}${simRef('D5','18',multiMeta.selected||[])}<div class="gap"></div><div></div><div></div><div></div><div></div>
      </div>
      <div class="l4-sim-status" id="l4SimMultiStatus">${escapeHtml(multiMeta.note)}</div>
    </div>`;
