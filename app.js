(() => {
  ...
  function renderSumifAverageif(panel){
    const st=getExState(3,{answers:{},guideSlide:0,readyToFinish:false,active:null});
    st.answers ||= {};
    st.guideSlide=Number.isInteger(st.guideSlide)?st.guideSlide:0;
    st.readyToFinish=!!st.readyToFinish;
    ...
    const guides=[
      {title:"1. SUMIF — satu kriteria",text:"SUMIF menjumlahkan angka berdasarkan satu kriteria. Pada lembar A, Jenis Item berada di kolom B dan Jumlah berada di kolom C.",formula:'=SUMIF($B$2:$B$21,E2,$C$2:$C$21)'},
      {title:"2. AVERAGEIF — satu kriteria",text:"AVERAGEIF menghitung rata-rata berdasarkan satu kriteria dengan susunan rentang kriteria, kriteria, lalu rentang nilai.",formula:'=AVERAGEIF($B$2:$B$21,E2,$C$2:$C$21)'},
      {title:"3. SUMIFS — beberapa kriteria",text:"SUMIFS menjumlahkan nilai ketika beberapa syarat terpenuhi. Pada lembar B, syaratnya adalah Jenis Item dan Cabang.",formula:'=SUMIFS($D$2:$D$21,$B$2:$B$21,F2,$C$2:$C$21,G2)'},
      {title:"4. AVERAGEIFS — beberapa kriteria",text:"AVERAGEIFS menghitung rata-rata nilai yang memenuhi lebih dari satu syarat. Urutan pertama selalu rentang nilai yang dirata-ratakan.",formula:'=AVERAGEIFS($D$2:$D$21,$B$2:$B$21,F2,$C$2:$C$21,G2)'},
      {title:"5. Ketik rumus lalu Enter",text:"Klik sel hasil, ketik rumus lengkap, lalu tekan Enter. Jika benar, sel menampilkan hasil dan rumus yang Anda gunakan.",formula:'Enter → periksa rumus → hasil tampil'}
    ];
    const guide=guides[Math.max(0,Math.min(guides.length-1,st.guideSlide))];

    const guideMini=`<div class="l4-guide-mini">
      <div class="l4-guide-mini-grid">
        ...
        <main class="l2-main">
          <section class="l2-learning-row">
            <div class="l2-material-card">
              ...
              <p><b>SUMIF</b> dan <b>AVERAGEIF</b> memakai satu kriteria. <b>SUMIFS</b> dan <b>AVERAGEIFS</b> dapat memakai dua atau lebih kriteria.</p>
              <div>
                <code>=SUMIF(range,kriteria,sum_range)</code>
                <code>=AVERAGEIF(range,kriteria,average_range)</code>
                <code>=SUMIFS(sum_range,criteria_range1,kriteria1,...)</code>
                <code>=AVERAGEIFS(avg_range,criteria_range1,kriteria1,...)</code>
              </div>
            </div>
