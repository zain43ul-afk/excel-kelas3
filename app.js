(() => {
  ...
  const CURRENT_PROGRESS_VERSION = "attempt-v3";
  ...
  function makeFreshRecordPreservingScores(cls,name,previous=null,sessionId=null){
    ...
    const saved=rec.exerciseScores.filter(v=>Number.isFinite(Number(v)));
  ...
  function normalizeStudentRecord(record){
    ...
    const oldScores=Array.from({length:8},(_,i)=>{
      ...
      if(Number.isFinite(Number(v))) return Math.max(40,Math.min(100,Math.round(Number(v))));
      const sec=oldSeconds[i];
      return oldCompleted[i] && Number.isFinite(Number(sec))
        ? Math.max(40,100-Math.floor(Number(sec)/60))
        : null;
    });

    // Migrasi otomatis dari versi lama.
    // Riwayat nilai dipertahankan, tetapi progress percobaan aktif dimulai 0.
    if(record.progressVersion !== CURRENT_PROGRESS_VERSION){
      ...
      const saved=oldScores.filter(v=>Number.isFinite(Number(v)));
    ...
    record.exerciseScores=Array.from({length:8},(_,i)=>{
      const v=record.exerciseScores?.[i];
      if(Number.isFinite(Number(v))) return Math.max(40,Math.min(100,Number(v)));
      const sec=record.exerciseCompletedSeconds?.[i];
      return record.completedExercises?.[i] && Number.isFinite(Number(sec))
        ? Math.max(40,100-Math.floor(Number(sec)/60))
        : null;
    });
    record.exerciseState ||= {};
  ...
  function getExerciseElapsed(index){
    ...
    if(done && Number.isFinite(Number(completed))) return Math.min(3600,Math.max(0,Number(completed)));
    const started=currentUser.exerciseStartedAt?.[index];
    if(!Number.isFinite(Number(started))) return 0;
  ...
  function finalExerciseScore(index){
    ...
    if(Number.isFinite(Number(saved))) return Math.max(40,Math.min(100,Number(saved)));
  ...
  function scoreForExercise(index){
    ...
    if(!Number.isFinite(Number(currentUser.exerciseStartedAt?.[index]))) return 0;
  ...
  function recomputeStudentAverageScore(){
    if(!currentUser) return;
    const scores=(currentUser.exerciseScores||[]).filter(v=>Number.isFinite(Number(v))).map(Number);
  ...
  async function autoFinishExpiredExercise(index){
    ...
    const started=currentUser.exerciseStartedAt?.[index];
    if(!Number.isFinite(Number(started))) return;
  ...
  function startTimer(){
    ...
    const tick=async()=>{
      ...
      try{
        ...
        for(let i=0;i<8;i++){
          if(!currentUser.completedExercises?.[i] && Number.isFinite(Number(currentUser.exerciseStartedAt?.[i])) && getExerciseElapsed(i)>=3600){
  ...
  function openExerciseWindow(i){
    ...
    if(!currentUser.completedExercises?.[i] && !Number.isFinite(Number(currentUser.exerciseStartedAt[i]))){
  ...
  async function completeExercise(i){
    ...
    if(!Number.isFinite(Number(currentUser.exerciseStartedAt[i]))){
      currentUser.exerciseStartedAt[i]=now();
    }
  ...
  function renderTeacher(){
    ...
    $("#teacherTableBody").innerHTML=classRecords.map(r=>{
      ...
      const perExerciseScores=Array.from({length:8},(_,i)=>{
        ...
        if(r.completedExercises?.[i] && Number.isFinite(Number(sec))){
          return liveScoreFromElapsed(sec);
        }
        return null;
      });
      ...
      const exerciseCells=perExerciseScores.map((score,i)=>{
        ...
        const sec=r.exerciseCompletedSeconds?.[i];
        const title=Number.isFinite(Number(sec))
      ...
      return `<tr>
        ...
        <td><button class="reset-btn" data-reset="${escapeHtml(progressKey(r.className,r.name))}" ${(!r.startedAt && !r.exerciseScores?.some(v=>Number.isFinite(Number(v))))?"disabled":""}>↶ Reset nilai</button></td>
      </tr>`;
    }).join("");
  ...
  function startNewMaterial(){
    if(!confirm("Mulai materi baru? Progres, timer, dan jawaban siswa akan direset. Nilai yang sudah tersimpan tetap dipertahankan.")) return;
    ...
    for(const [cls,names] of Object.entries(ROSTER)){
      for(const name of names){
        const previous=getStudent(cls,name);
        const rec=makeFreshRecordPreservingScores(cls,name,previous,newSession);
        rec.progressVersion=CURRENT_PROGRESS_VERSION;
        preservedProgress[safeId(progressKey(cls,name))]=rec;
      }
    }

    allProgress=preservedProgress;
    settings.materialFinished=false;
    settings.sessionId=newSession;
    settings.unlocked=[true,false,false,false,false,false,false,false];

    saveLocal();

    if(realtime){
      db.ref("kelasExcel/progress").set(preservedProgress);
      db.ref("kelasExcel/settings").set(settings);
    }

    renderTeacher();
    toast("Materi baru dimulai. Progres dan timer direset, nilai lama tetap tersimpan.");
  }
