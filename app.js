(() => {
  ...
  function menuScoreForExercise(index){
    return currentUser?.completedExercises?.[index] ? finalExerciseScore(index) : 0;
  }
  ...
  async function completeExercise(i){
    ...
    closeExerciseWindow();
    renderStudent();
    toast(`Latihan ${i+1} selesai. Nilai akhir: ${currentUser.exerciseScores[i]}.`);
  }
  ...
  function renderTeacher(){
    ...
    if(recapDesc) recapDesc.textContent=isProgressMode
      ...
      : "Nilai setiap latihan ditampilkan pada kolom 1–8 · kolom Nilai adalah rata-rata latihan yang sudah selesai";
    ...
    $("#teacherTableBody").innerHTML=classRecords.map(r=>{
      ...
      const perExerciseScores=Array.from({length:8},(_,i)=>{
        if(!r.completedExercises?.[i]) return null;
        const saved=r.exerciseScores?.[i];
        if(Number.isFinite(Number(saved))) return Math.max(40,Math.min(100,Math.round(Number(saved))));
        const sec=r.exerciseCompletedSeconds?.[i];
        return Number.isFinite(Number(sec))?liveScoreFromElapsed(sec):40;
      });
      ...
      const exerciseCells=perExerciseScores.map((score,i)=>{
        ...
        if(teacherRecapMode==="nilai"){
          if(!completed || score===null) return `<td class="exercise-score-cell">—</td>`;
          return `<td class="exercise-score-cell">
            <span class="teacher-exercise-score" title="Nilai Latihan ${i+1}: ${score}">${score}</span>
          </td>`;
        }
