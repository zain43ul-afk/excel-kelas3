(() => {
  ...
  const GAME_VERSION = "team-v3-bank-50";
  ...
  const GAME = {
    ...
    scenario:null, steps:[], assigned:[], board:{}, meta:{}, selected:null,
  ...
  function render(){
    ...
    <aside class="alg-side">
      <section class="alg-card alg-side-card">
        <h4>👥 Komposisi Kelompok</h4>${members}
        <div class="alg-wait">${size} siswa × 4 kartu = <b>${GAME.steps.length} langkah</b>.</div>
      </section>
      <section class="alg-card alg-side-card">
      ...
      <section class="alg-card alg-side-card">
        ...
        <div class="alg-rule">
          ...
          <b>5.</b> Setiap kelompok mendapat algoritma berbeda dari bank 50 soal.<br>
          <b>6.</b> Bank diacak ulang saat sesi/materi baru dimulai.<br>
          <b>7.</b> Setiap siswa selalu memegang tepat 4 kartu.
        </div>
      </section>
    </aside></div>`;
  ...
  function connect(){
    disconnect();
    if(GAME.demo||!window.firebase?.database){GAME.board={};GAME.meta={members:{}};render();return}
  ...
  function mount(panel){
    injectCss();setup();GAME.panel=panel;GAME.selected=null;GAME.completed=false;
