(() => {
  ...
  const GAME_VERSION = "team-v8-scoped-observers";
  ...
  const CSS = `
    ...
    .alg-team-body{padding:18px;display:grid;grid-template-columns:minmax(0,1fr) 300px;gap:16px;align-items:start}
    .alg-main{min-width:0}
    .alg-side{display:flex;flex-direction:column;gap:14px;position:sticky;top:14px}
    .alg-card{background:#fff;border:1px solid #dbe4f0;border-radius:18px;box-shadow:0 10px 28px rgba(15,23,42,.05);overflow:hidden}
    .alg-card-pad{padding:16px 18px}
    .alg-hero{padding:18px 20px;background:linear-gradient(135deg,#eef2ff,#f8fafc);border:1px solid #c7d2fe;border-radius:18px}
    .alg-hero h3{margin:0 0 6px;font-size:20px;color:#312e81}
    .alg-hero p{margin:0;color:#475569;line-height:1.55;font-size:13px}
    .alg-badges{display:flex;flex-wrap:wrap;gap:8px;margin-top:12px}
    .alg-badge{padding:7px 10px;border-radius:999px;background:#fff;border:1px solid #c7d2fe;font-size:11px;font-weight:900;color:#4338ca}
    .alg-subtitle{display:flex;align-items:center;gap:8px;margin:0 0 10px;color:#0f172a;font-size:16px}
    .alg-subtitle small{font-size:12px;color:#64748b;font-weight:700}
    .alg-member-strip{padding:16px 18px;margin-top:14px}
    .alg-member-strip-head{display:flex;justify-content:space-between;align-items:center;gap:10px;margin-bottom:12px}
    .alg-member-strip-head h4{margin:0;font-size:16px;color:#0f172a}
    .alg-member-strip-head span{font-size:11px;font-weight:900;color:#4338ca;background:#eef2ff;border:1px solid #c7d2fe;border-radius:999px;padding:6px 10px}
    .alg-member-row{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:10px}
    .alg-member-chip{display:flex;align-items:center;gap:10px;padding:12px;border:1px solid #dbe4f0;border-radius:14px;min-width:0}
    .alg-member-chip .alg-avatar{width:32px;height:32px;border-radius:50%;display:grid;place-items:center;font-weight:900;font-size:13px;flex:0 0 32px}
    .alg-member-chip b{display:block;font-size:13px;color:#0f172a;line-height:1.2;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    .alg-member-chip small{display:block;font-size:11px;color:#64748b;line-height:1.35;margin-top:2px}
    .alg-member-chip.member-0{background:#eff6ff;border-color:#bfdbfe}
    .alg-member-chip.member-0 .alg-avatar{background:#2563eb;color:#fff}
    .alg-member-chip.member-1{background:#f0fdf4;border-color:#bbf7d0}
    .alg-member-chip.member-1 .alg-avatar{background:#16a34a;color:#fff}
    .alg-member-chip.member-2{background:#fff7ed;border-color:#fed7aa}
    .alg-member-chip.member-2 .alg-avatar{background:#ea580c;color:#fff}
    .alg-member-chip.member-3{background:#faf5ff;border-color:#e9d5ff}
    .alg-member-chip.member-3 .alg-avatar{background:#9333ea;color:#fff}
    .alg-member-chip.member-4{background:#fef2f2;border-color:#fecaca}
    .alg-member-chip.member-4 .alg-avatar{background:#dc2626;color:#fff}
    .alg-member-chip.member-5{background:#ecfeff;border-color:#a5f3fc}
    .alg-member-chip.member-5 .alg-avatar{background:#0891b2;color:#fff}

    .alg-section{margin-top:14px}
    .alg-section-head{display:flex;justify-content:space-between;gap:12px;align-items:flex-start;margin-bottom:12px}
    .alg-section-head h4{margin:0;font-size:17px;color:#0f172a}
    .alg-section-head p{margin:4px 0 0;color:#64748b;font-size:12px;line-height:1.5}
    .alg-counter{font-size:13px;font-weight:900;color:#0f172a}
    .alg-helper{padding:10px 12px;border-radius:12px;background:#eff6ff;border:1px solid #bfdbfe;color:#1d4ed8;font-size:12px;font-weight:800;margin-top:10px}
    ...
    .alg-my-card:hover{transform:translateY(-1px);border-color:#6366f1}
    .alg-my-card.selected{border-color:#4f46e5;background:#e0e7ff;box-shadow:0 0 0 3px rgba(99,102,241,.12)}
    .alg-my-card.placed{opacity:.45;cursor:default}
    .alg-my-card b{display:block;color:#4338ca;font-size:10px;margin-bottom:6px;font-weight:900;letter-spacing:.03em}
    .alg-my-card span{font-weight:800;line-height:1.4;font-size:13px}
    .alg-board{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}
    .alg-slot{min-height:74px;border:1.5px dashed #94a3b8;background:#f8fafc;border-radius:12px;padding:10px;display:grid;grid-template-columns:32px 1fr;gap:9px;align-items:start;cursor:pointer}
    .alg-slot:hover{border-color:#6366f1;background:#f5f3ff}
    .alg-slot.filled{border-style:solid;background:#fff}
    .alg-slot.mine{border-color:#818cf8;background:#eef2ff}
    .alg-slot.correct-final{border-color:#22c55e;background:#f0fdf4}
    .alg-slot-no{width:28px;height:28px;border-radius:9px;background:#e2e8f0;display:grid;place-items:center;font-weight:900;color:#475569;font-size:12px}
    .alg-slot.mine .alg-slot-no{background:#4f46e5;color:#fff}
    .alg-slot-text{font-size:12px;font-weight:800;line-height:1.4;color:#1e293b;display:block}
    .alg-slot-owner{display:block;font-size:10px;color:#64748b;font-weight:700;margin-top:4px}
    .alg-actions{display:flex;gap:10px;flex-wrap:wrap;margin-top:14px}
    .alg-btn{border:0;border-radius:12px;padding:10px 14px;font-weight:900;cursor:pointer;font-size:13px}
    .alg-btn.primary{background:#4f46e5;color:#fff}
    .alg-btn.secondary{background:#e2e8f0;color:#334155}
    .alg-btn.success{background:#16a34a;color:#fff}
    .alg-btn:disabled{opacity:.45;cursor:not-allowed}
    .alg-feedback{margin-top:12px;padding:12px 14px;border-radius:12px;background:#fff7ed;border:1px solid #fed7aa;color:#9a3412;font-weight:800;font-size:12px;line-height:1.5}
    .alg-feedback.ok{background:#f0fdf4;border-color:#bbf7d0;color:#166534}

    .alg-chat-card{padding:16px;display:flex;flex-direction:column;min-height:420px}
    .alg-chat-head{display:flex;justify-content:space-between;gap:8px;align-items:flex-start;margin-bottom:10px}
    .alg-chat-head h4{margin:0;font-size:16px;color:#0f172a}
    .alg-chat-head p{margin:4px 0 0;font-size:12px;color:#64748b;line-height:1.45}
    .alg-chat-list{flex:1;min-height:240px;max-height:360px;overflow:auto;border:1px solid #dbe4f0;border-radius:14px;background:#f8fafc;padding:10px;display:flex;flex-direction:column;gap:8px}
    .alg-chat-empty{padding:14px;text-align:center;color:#94a3b8;font-size:12px;line-height:1.45}
    .alg-msg{max-width:92%;padding:10px 12px;border-radius:14px;background:#fff;border:1px solid #dbe4f0;box-shadow:0 2px 8px rgba(15,23,42,.03)}
    .alg-msg.mine{align-self:flex-end}
    .alg-msg.member-0{background:#eff6ff;border-color:#bfdbfe}
    .alg-msg.member-1{background:#f0fdf4;border-color:#bbf7d0}
    .alg-msg.member-2{background:#fff7ed;border-color:#fed7aa}
    .alg-msg.member-3{background:#faf5ff;border-color:#e9d5ff}
    .alg-msg.member-4{background:#fef2f2;border-color:#fecaca}
    .alg-msg.member-5{background:#ecfeff;border-color:#a5f3fc}
    .alg-msg-meta{display:flex;justify-content:space-between;gap:8px;align-items:center;margin-bottom:4px}
    .alg-msg-meta-left{display:flex;align-items:center;gap:6px;min-width:0}
    .alg-msg-meta b{font-size:12px;color:#0f172a;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    .alg-msg-time{font-size:10px;color:#94a3b8;white-space:nowrap}
    .alg-role-label{display:inline-flex;align-items:center;padding:2px 6px;border-radius:999px;font-size:9px;font-weight:900;background:#334155;color:#fff}
    .alg-msg.member-0 .alg-role-label{background:#2563eb}
    .alg-msg.member-1 .alg-role-label{background:#16a34a}
    .alg-msg.member-2 .alg-role-label{background:#ea580c}
    .alg-msg.member-3 .alg-role-label{background:#9333ea}
    .alg-msg.member-4 .alg-role-label{background:#dc2626}
    .alg-msg.member-5 .alg-role-label{background:#0891b2}
    .alg-msg-text{font-size:12px;color:#334155;line-height:1.45;white-space:pre-wrap;word-break:break-word}
    .alg-chat-form{display:grid;grid-template-columns:1fr auto;gap:8px;margin-top:10px}
    .alg-chat-form textarea{resize:none;min-height:74px;max-height:130px;border:1px solid #cbd5e1;border-radius:12px;padding:11px 12px;font:inherit;line-height:1.45}
    .alg-chat-form textarea:focus{outline:none;border-color:#6366f1;box-shadow:0 0 0 3px rgba(99,102,241,.12)}
    .alg-chat-form .alg-btn{align-self:end;min-width:72px}

    .alg-side-card{padding:14px 16px}
    .alg-side-card h4{margin:0 0 10px;font-size:16px;color:#0f172a}
    .alg-stat-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}
    .alg-stat{padding:10px;border-radius:12px;background:#f8fafc;border:1px solid #e2e8f0}
    .alg-stat strong{display:block;font-size:20px;color:#312e81;line-height:1.1}
    .alg-stat span{font-size:10px;color:#64748b;font-weight:800;margin-top:4px;display:block}
    .alg-rule{font-size:12px;color:#475569;line-height:1.6}
    .alg-rule b{color:#1e293b}

    .alg-teacher-monitor{margin-top:18px;background:#fff;border:1px solid #dbe4f0;border-radius:18px;box-shadow:0 10px 28px rgba(15,23,42,.05);overflow:hidden}
    .alg-teacher-monitor-head{padding:18px 20px;border-bottom:1px solid #e2e8f0;display:flex;justify-content:space-between;gap:14px;align-items:flex-start}
    .alg-teacher-monitor-head h3{margin:0 0 5px;color:#0f172a;font-size:18px}
    .alg-teacher-monitor-head p{margin:0;color:#64748b;font-size:12px;line-height:1.45}
    .alg-teacher-live{display:inline-flex;align-items:center;gap:7px;padding:7px 10px;border-radius:999px;background:#ecfdf5;border:1px solid #a7f3d0;color:#047857;font-size:11px;font-weight:900;white-space:nowrap}
    .alg-teacher-live i{width:8px;height:8px;border-radius:50%;background:#10b981;box-shadow:0 0 0 4px rgba(16,185,129,.12)}
    .alg-teacher-group-tabs{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:8px;padding:14px 18px;background:#f8fafc;border-bottom:1px solid #e2e8f0}
    .alg-teacher-group-tab{border:1px solid #cbd5e1;background:#fff;border-radius:12px;padding:10px;cursor:pointer;text-align:left}
    .alg-teacher-group-tab.active{border-color:#4f46e5;background:#eef2ff;box-shadow:0 0 0 2px rgba(99,102,241,.10)}
    .alg-teacher-group-tab b{display:block;color:#0f172a;font-size:12px}
    .alg-teacher-group-tab span{display:block;color:#64748b;font-size:10px;margin-top:3px}
    .alg-teacher-group-tab em{font-style:normal;display:inline-grid;place-items:center;min-width:22px;height:22px;padding:0 6px;margin-top:7px;border-radius:999px;background:#e2e8f0;color:#475569;font-size:10px;font-weight:900}
    .alg-teacher-group-tab.active em{background:#4f46e5;color:#fff}
    .alg-teacher-monitor-body{display:grid;grid-template-columns:minmax(0,1fr) 260px}
    .alg-teacher-chat{padding:16px;border-right:1px solid #e2e8f0}
    .alg-teacher-chat-list{height:360px;overflow:auto;background:#f8fafc;border:1px solid #e2e8f0;border-radius:14px;padding:12px;display:flex;flex-direction:column;gap:9px}
    .alg-teacher-msg{max-width:92%;padding:10px 12px;border:1px solid #dbe4f0;background:#fff;border-radius:14px}
    .alg-teacher-msg.member-0{background:#eff6ff;border-color:#bfdbfe}
    .alg-teacher-msg.member-1{background:#f0fdf4;border-color:#bbf7d0}
    .alg-teacher-msg.member-2{background:#fff7ed;border-color:#fed7aa}
    .alg-teacher-msg.member-3{background:#faf5ff;border-color:#e9d5ff}
    .alg-teacher-msg.member-4{background:#fef2f2;border-color:#fecaca}
    .alg-teacher-msg.member-5{background:#ecfeff;border-color:#a5f3fc}
    .alg-teacher-msg-head{display:flex;justify-content:space-between;gap:10px;align-items:center;margin-bottom:4px}
    .alg-teacher-msg-head-left{display:flex;align-items:center;gap:6px;min-width:0}
    .alg-teacher-msg-head b{font-size:12px;color:#0f172a}
    .alg-teacher-msg-head span{font-size:10px;color:#94a3b8}
    .alg-teacher-msg-text{font-size:12px;line-height:1.45;color:#334155;white-space:pre-wrap;word-break:break-word}
    .alg-teacher-empty{height:100%;display:grid;place-items:center;text-align:center;color:#94a3b8;font-size:12px;padding:20px}
    .alg-teacher-info{padding:16px}
    .alg-teacher-info h4{margin:0 0 12px;font-size:14px;color:#0f172a}
    .alg-teacher-info-card{border:1px solid #e2e8f0;border-radius:12px;padding:11px 12px;margin-bottom:9px;background:#f8fafc}
    .alg-teacher-info-card small{display:block;color:#64748b;font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.04em}
    .alg-teacher-info-card b{display:block;color:#1e293b;font-size:12px;line-height:1.4;margin-top:4px}
    .alg-teacher-member-legend{display:flex;flex-direction:column;gap:7px}
    .alg-teacher-legend-row{display:flex;align-items:center;gap:8px;font-size:11px;color:#475569}
    .alg-teacher-legend-dot{width:11px;height:11px;border-radius:50%}
    .alg-teacher-legend-row.member-0 .alg-teacher-legend-dot{background:#2563eb}
    .alg-teacher-legend-row.member-1 .alg-teacher-legend-dot{background:#16a34a}
    .alg-teacher-legend-row.member-2 .alg-teacher-legend-dot{background:#ea580c}
    .alg-teacher-legend-row.member-3 .alg-teacher-legend-dot{background:#9333ea}
    .alg-teacher-legend-row.member-4 .alg-teacher-legend-dot{background:#dc2626}
    .alg-teacher-legend-row.member-5 .alg-teacher-legend-dot{background:#0891b2}

    @media(max-width:1100px){
      .alg-team-body{grid-template-columns:1fr}
      .alg-side{position:static}
      .alg-chat-card{min-height:360px}
    }
    @media(max-width:900px){
      .alg-board,.alg-my-cards{grid-template-columns:1fr}
      .alg-teacher-monitor-body{grid-template-columns:1fr}
      .alg-teacher-chat{border-right:0;border-bottom:1px solid #e2e8f0}
  ...
  function render(){
    ...
    const memberStrip=GAME.members.map((m,i)=>`
      <div class="alg-member-chip member-${i}">
        <span class="alg-avatar">${i+1}</span>
        <div>
          <b>${esc(m)}</b>
          <small>${m===GAME.student.name?"Anda · ":""}Anggota ${i+1} · 4 kartu</small>
        </div>
      </div>`).join("");
    ...
    const board=Array.from({length:GAME.steps.length},(_,i)=>{
      ...
      return `<div class="alg-slot ${x?"filled":""} ${mine?"mine":""} ${GAME.completed?"correct-final":""}" data-slot="${i}">
        ...
        <div>
          ${x?`<span class="alg-slot-text">${esc(GAME.steps[x.stepId]?.t||"")}</span><small class="alg-slot-owner">${mine?"Kartu Anda":`oleh ${esc(x.owner||"anggota")}`}</small>`:`<span class="alg-slot-text" style="color:#94a3b8">Klik untuk menempatkan kartu</span>`}
        </div>
      </div>`;
    }).join("");
    ...
    const chatHtml=(GAME.chat||[]).length
      ? GAME.chat.map(msg=>{
          ...
          return `<div class="alg-msg member-${mi} ${msg.name===GAME.student.name?"mine":""}">
            <div class="alg-msg-meta">
              <div class="alg-msg-meta-left">
                <b>${esc(msg.name||"Anggota")}</b>
                <span class="alg-role-label">Anggota ${mi+1}</span>
              </div>
      ...
      : '<div class="alg-chat-empty">Belum ada pesan. Gunakan chat ini untuk menyampaikan isi kartu, mendiskusikan urutan, atau memberi usulan revisi.</div>';
        ...
        <main class="alg-main">
          <section class="alg-hero">
            ...
            <div class="alg-badges">
              ...
              <span class="alg-badge">Bank 50 algoritma</span>
              <span class="alg-badge">Diacak tiap sesi</span>
            </div>
          </section>
          ...
          <section class="alg-card alg-card-pad alg-section">
            <div class="alg-section-head">
              <div>
                <h4>1. Empat kartu Anda</h4>
                <p>Seperti pada game diskusi, setiap anggota hanya menguasai sebagian informasi. Sampaikan isi kartu Anda di chat kelompok atau diskusi langsung, lalu sepakati posisinya.</p>
              </div>
          ...
          <section class="alg-card alg-card-pad alg-section">
            <div class="alg-section-head">
              <div>
                ...
                <p>Semua anggota kelompok melihat papan yang sama. Tekan <b>Uji Algoritma</b> setelah semua slot terisi.</p>
              </div>
              <div class="alg-counter">${Object.keys(GAME.board).length}/${GAME.steps.length} terisi</div>
            </div>
        ...
        <aside class="alg-side">
          <section class="alg-card alg-chat-card">
            <div class="alg-chat-head">
              <div>
                <h4>💬 Chat Kelompok</h4>
                <p>Sistem diskusi seperti game kelas 2. Gunakan chat untuk memberi informasi, menyanggah, atau mengusulkan posisi kartu.</p>
              </div>
          ...
          <section class="alg-card alg-side-card">
            ...
            <div class="alg-rule">
              ...
              <b>2.</b> Setiap siswa selalu memegang 4 kartu.<br>
              <b>3.</b> Kelompok 4 siswa = 16 langkah, 5 siswa = 20 langkah, 6 siswa = 24 langkah.<br>
              <b>4.</b> Setiap kelompok mendapat algoritma berbeda dari bank 50 soal.<br>
              <b>5.</b> Gunakan chat kelompok untuk menyampaikan isi kartu dan berdiskusi.<br>
              <b>6.</b> Hanya pemilik kartu yang dapat memindahkan kartunya sendiri.
            </div>
          </section>
        </aside>
      </div>`;
    ...
    return messages.map(msg=>{
      ...
      return `<div class="alg-teacher-msg member-${idx}">
        <div class="alg-teacher-msg-head">
          <div class="alg-teacher-msg-head-left"><b>${esc(msg.name||"Anggota")}</b><span class="alg-role-label">Anggota ${idx+1}</span></div>
  ...
  function renderTeacherMonitor(){
    ...
    const legend=members.length
      ? members.map((m,i)=>`<div class="alg-teacher-legend-row member-${i}"><span class="alg-teacher-legend-dot"></span><span><b>${esc(m)}</b> · Anggota ${i+1}</span></div>`).join("")
  ...
  function ensureTeacherMonitor(){
    ...
    // CSS chat/monitor sebelumnya baru masuk saat latihan Algoritma dibuka.
    // Guru harus tetap mendapat style monitor meskipun belum pernah membuka latihan 7.
    injectCss();
    ...
    // PENTING:
    // Jangan render ulang monitor pada setiap MutationObserver.
    // renderTeacherMonitor() mengubah innerHTML dan memicu MutationObserver lagi.
    // Versi lama membuat loop render tanpa akhir sehingga halaman login guru freeze.
  ...
  function scheduleExerciseMount(){
    ...
    requestAnimationFrame(()=>{
      ...
      if(visible && (title==="Algoritma"||title==="Algoritma Kelompok") && !already){
        mount(panel);
      }
    });
  }
