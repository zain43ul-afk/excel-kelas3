(() => {
  ...
  const GAME_VERSION = "team-v6-teacher-freeze-fix";
  ...
  new MutationObserver(mutations=>{
    // Perubahan isi monitor guru dan chat siswa tidak perlu memicu proses mount ulang.
    const meaningful=mutations.some(m=>{
      const target=m.target?.nodeType===1 ? m.target : m.target?.parentElement;
      if(!target) return true;
      if(target.closest?.("#algorithmTeacherMonitor")) return false;
      if(target.closest?.(".alg-chat-list")) return false;
      return true;
    });
    if(meaningful) safeWatch();
  }).observe(document.documentElement,{childList:true,subtree:true});

  // Pastikan monitor muncul segera setelah login guru tanpa bergantung pada mutation loop.
  document.addEventListener("click",e=>{
    if(e.target.closest?.("#teacherLoginBtn")){
      setTimeout(()=>{ensureTeacherMonitor();},250);
      setTimeout(()=>{ensureTeacherMonitor();},700);
    }
  });

  // Fallback ringan: hanya memastikan host monitor ada / kelas berubah.
  // Tidak melakukan render jika keadaan tidak berubah.
  setInterval(()=>{
    const teacherView=document.getElementById("teacherView");
    if(teacherView?.classList.contains("active")) ensureTeacherMonitor();
  },1200);

  window.addEventListener("load",()=>setTimeout(safeWatch,250));
})();