(() => {
  ...
  const GAME_VERSION = "team-v9-student-layout-match-example";
  ...
  function render(){
    ...
    const chatHtml=(GAME.chat||[]).length
      ? GAME.chat.map(msg=>{
          ...
          const initials=(msg.name||"A").split(/\s+/).slice(0,2).map(x=>x[0]||"").join("").toUpperCase();
          return `<div class="alg-msg member-${mi} ${msg.name===GAME.student.name?"mine":""}">
            ...
            <div class="alg-msg-body">
              <div class="alg-msg-head">
                <div class="alg-msg-title">${esc(msg.name||"Anggota")} <span class="role">(Anggota ${mi+1})</span></div>
