...
(() => {
  ...
  const VERSION = "algorithm-race-v12";
  ...
  function replaceRequired(src, search, replacement, label){
    if(!src.includes(search)){
      throw new Error(`Patch gagal: bagian "${label}" tidak ditemukan pada algorithm-team-game.js.`);
    }
...
`;
  ...
  async function load(){
    try{
      ...
      const oldCorrect=String.raw`    if(wrong<0){
      GAME.completed=true;
      ...
      const newCorrect=String.raw`    if(wrong<0){
      if(raceIsActive()){
      ...
      const oldListener=String.raw`      GAME.meta=v;
      ...
      render();`;
      const newListener=String.raw`      GAME.meta=v;
      ...
      const oldRoot=String.raw`    GAME.rootRef=firebase.database().ref(\`kelasExcel/algorithmTeamV3/\${safeId(GAME.student.className)}/\${safeId(currentSessionId())}/group_\${GAME.groupNo}\`);`;
      ...
      src=replaceRequired(
        ...
        '    render();connect();listenRaceManager();startRaceTicker();\n    panel.dataset.algMounting="";',
        "mount race"
      );
